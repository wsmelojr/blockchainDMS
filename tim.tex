...
\subsection{Implementing vehicle speed meters a distributed measuring system}

\begin{figure}[!t]
\centering
\includegraphics[width=.5\textwidth]{speedmeter} %[width=2.5in]
...
Our experiment uses Hyperledger Fabric (HLF) \cite{Androulaki2018} for developing a permissioned blockchain where the peers cooperate to store measurements and execute LR software.
...
Endorsers are peers which effectively execute smart contracts, which are called chaincodes in HLF.
...
We set up our vehicle speed meter DMS as follows.
...
Our experiment supposes the implementation of legal metrology activities as follows.
...
In HLF, a chaincode instantiation means to notify the endorsers that they may execute that chaincode.
...
\begin{itemize}
   \item{The throughput and latency of each individual endorser peer. This aspect is important because it helps to estimate how many peers a speed meter owner will need, considering a specific demand.}
...
The test environment consists of 4 nodes from Dell PowerEdge R410 Cluster.
Each node has two CPUs Intel® Xeon® Processor E5520 with 2.27 GHz and 32 GB of RAM.
HLF runs over \emph{docker containers}, so each physical node can host several peers.
For practical reasons, we use three nodes for allocating the consensus (BFT quorum???) service, LaSIGE peers and Inmetro peers.
The last node allocates containers that simulate the speed meter hardware.
...
Each test run starts a load of $n$ concurrent clients.
These clients select a vector of bytes containing the sensors raw data from the dataset mentioned above.
Clients use HLF protocol to create transactions and send such data to an endorser peer from LaSIGE organization.
Each client does that continually, sending as many transactions as possible.
The clients also keep records of timestamps and the elapsed time between sending the transaction and receiving an endorsed package.
Such information gives the endorser peer throughput and latency estimative.
In turn, an independent peer in Inmetro organization gives us the blockchain throughput estimative. 
