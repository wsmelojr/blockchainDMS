...
% in the abstract
\begin{abstract}
In the last years, measuring instruments have become quite complex due to the integration of embedded systems and software components and the increasing aggregation of new features. 
...
In this paper, we evaluate the use of blockchains as a resource to overcome such challenges.
...
We work with the hypothesis that the aforementioned difficulties should be overcome with alternative approaches that simplify MI design while employing strategies for decentralizing metrological supervision. 
...
More recently, a few works have proposed blockchain applications in legal metrology, which include decentralized audit, mechanisms for software loading, PKI for MI manufacturers and Distributed Measuring Systems (DMS) \cite{Peters2018,MeloJr.2017}.
...
This paper extends the ideas presented in our previous work under the title ``How Blockchains can improve Measuring Instruments Regulation and Control'' \cite{MeloJr.2018a}. 
...
Usually, legal metrology regulations are defined by government agencies or international committees. Once a regulation directive is approved, it traditionally establishes a set of requirements and activities for providing MI metrological assurance \cite{RodriguesFilho2015}. Activities are classified in two levels. The first level is related to legal control and includes MI type approval, validation and verification. The second level activities covers metrological supervision, including quality, market and field surveillance. Activities can be executed by different parties according to the directives adopted in regulation. Usually, notified bodies\footnote{Notified bodies are public or private parties organized for verifying MI.} are designated to assert MI conformity in both activity levels \cite{Esche2015,Oppermann2016}. 
...
A communication interface ables the MI to send sensing raw data to the cloud measuring system. 
...
This idea fits very well in the blockchain architecture. 
...
That motivates new players to compete by more efficient measuring algorithms once as faster they execute, more credits they can earn.
...
First, a manufacturer needs to reveal his LR software only to peers contracted to execute his measuring chaincode as a service, and to the notified body responsible for approving it.
...
Security policies can also work as a protection mechanism against collusion attacks and also implement metrological surveillance.
...
The solo orderer service is native from Fabric distribution and it is very practical to implement tests and make proof of concept.
...
Such approach results in a decentralized consensus, thus providing security against collusion attacks.
...
Such an approach improves performance and eliminates privacy concerns.
...
That enables any MI in the field to generate transactions, ask endorsers to determine the vehicle speed, and therefore store the vehicle speed legally relevant information in the blockchain ledger.
...
Consequently, any attempt at violating seals or stealing a private key shall flaw the speed meter hardware, forcing its replacement and exposing the attack try.

\subsubsection{An attacker tries to install a malicious chaincode in an endorser peer}
Since the blockchain ledger already contains the chaincode fingerprint of every instantiated chaincode, a legitimate endorser automatically checks the LR chaincode integrity on deploy time.
...
Thus, a collusion attack including only the LaSIGE organization does not succeed.
...
We try to estimate the blockchain peers best behavior without considering network communication issues.
...
That is done on purpose to evaluate the real results that a speed meter solution owner can obtain by using Fabric.
...
The transactions simulation uses real data from speed meters developed by the company Perkons SA (www.perkons.com), which kindly granted a dataset for this experiment.
...
Each client instance corresponds to a container process that sends transactions to the blockchain continually, which requires massive hardware resources.
...
Figures \ref{f:latth} and \ref{f:latthbft4} depict our tests results.
...
It reaches the best trade-off with throughput around 380 tps and latency in 2 seconds with the same workload of 600 simultaneous clients.
...
Although the number of replicas impacts performance, it aggregates security by preventing collusion attacks.
