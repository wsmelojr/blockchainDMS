...
%DIF DEL tim-b4.tex   Fri Jan 11 22:59:32 2019
%DIF ADD tim.tex      Sat Jan 12 19:07:28 2019
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
\author{Wilson~S.~Melo~Jr, %~\IEEEmembership{Member,~IEEE,}
        ...
        Altair~Santin
        and Luiz~F.\DIFaddbegin \DIFadd{~}\DIFaddend R.\DIFaddbegin \DIFadd{~}\DIFaddend C.~Carmo
\DIFdelbegin %DIFDELCMD < \thanks{W. Melo Jr. and L.F.R.C. Carmo are with the National Institute of Metrology, Quality and Technology, Duque de Caxias, RJ, Brazil; and with the Federal University of Rio de Janeiro, Brazil.}%%%
\DIFdelend \DIFaddbegin \thanks{W. Melo Jr. and Luiz F. R. C. Carmo are with the National Institute of Metrology, Quality and Technology, Duque de Caxias, RJ, Brazil; and with the Federal University of Rio de Janeiro, Brazil.}\DIFaddend % <-this % stops a space
...
\DIFdelbegin %DIFDELCMD < \markboth{How Blockchains can improve Measuring Instruments Regulation and Control (extended version)}{}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \markboth{Using Blockchains to Implement Distributed Measuring Systems}{}
\DIFaddend %{Melo Jr. \MakeLowercase{\textit{et al.}}: How Blockchains can improve Measuring Instruments Regulation and Control (extended version)}
...
\DIFdelbegin \DIFdel{In the sequence, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend implement a vehicle speed measuring system using the Hyperledger Fabric blockchain platform.
...
Finally, we point out the main challenges related to \DIFdelbegin \DIFdel{such an }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend approach, suggesting alternatives and potential issues \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{to be addressed by }\DIFaddend future works.
\end{abstract}
...
MI also can be seen as \DIFdelbegin \DIFdel{elementary }\DIFdelend \DIFaddbegin \DIFadd{fundamental }\DIFaddend building blocks for new technologies such as internet of things and cyber physical systems (e.g., smart grids) \cite{RodriguesFilho2015,Esche2015,Camara2012,Boccardo2014,Peters2015,Oppermann2018}. 
...
%That creates
This kind of scenario might expose MI to security gaps \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend can be explored with malicious intent \cite{Boccardo2014,Peters2015}.
...
Type approval requires more \DIFdelbegin \DIFdel{efforts }\DIFdelend \DIFaddbegin \DIFadd{effort }\DIFaddend while verification can involve use cases which are hard to reproduce inside labs. 
In turn, metrological supervision becomes difficult due to the diversity of MI models, their \DIFdelbegin \DIFdel{geographically spread over deployment }\DIFdelend \DIFaddbegin \DIFadd{geographic spread }\DIFaddend and the limited resources owned by regulatory agencies.
...
A blockchain can be described as a distributed data structure which assures information integrity and authenticity while providing a platform for executing \DIFdelbegin \DIFdel{self-enforce }\DIFdelend \DIFaddbegin \DIFadd{self-enforced }\DIFaddend software procedures, called \emph{smart contracts} \cite{Christidis2016}. Blockchain solutions have been very successful in financial applications (e.g., Bitcoin and Ethereum), \DIFdelbegin \DIFdel{which motivates innovative ideas using blockchains }\DIFdelend \DIFaddbegin \DIFadd{inspiring its use }\DIFaddend in different applications and knowledge areas \cite{Nakamoto2008,Christidis2016}.
More recently, a few works have proposed blockchain applications in legal metrology, which include decentralized audit, mechanisms for software loading, \DIFdelbegin \DIFdel{PKI }\DIFdelend \DIFaddbegin \DIFadd{Public Key infrastructure (PKI) }\DIFaddend for MI manufacturers and Distributed Measuring Systems (DMS) \cite{Peters2018,MeloJr.2018a}.
...
Then we explore ideas related to the integration of MI in \DIFdelbegin \DIFdel{Distributed Measuring Systems (DMS)}\DIFdelend \DIFaddbegin \DIFadd{DMS}\DIFaddend , proposing a blockchain-based model.
Such aspects result in an innovative concept that dissociates the measurement service from the measurement quantity while \DIFaddbegin \DIFadd{it }\DIFaddend improves MI security and makes metrological assurance simpler and less expensive. 
...
\begin{itemize}
 ...
 \item We propose an architectural model for implementing our idea, showing that MI and blockchains enable a new business model where the measuring process is an independent service, \DIFdelbegin \DIFdel{which reduces conflict of interests}\DIFdelend \DIFaddbegin \DIFadd{reducing conflicts of interest}\DIFaddend .
...
Legal metrology embraces MI regulation and control. It is crucial to assure the \DIFdelbegin \DIFdel{measurements correctness }\DIFdelend \DIFaddbegin \DIFadd{correctness of measurements }\DIFaddend \cite{RodriguesFilho2015} and regulate consumer relations \cite{Oppermann2018}.
...
The main ones are related to legal control \DIFaddbegin \DIFadd{of }\DIFaddend MI and type approval, which can include documentation and code inspection, validation and verification; and metrological supervision, including quality, market and field surveillance.
...
The \emph{OIML \DIFdelbegin \footnote{\DIFdel{OIML is the International Organization of Legal Metrology.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend D 31(E)} document \cite{InternationalOrganizationofLegalMetrologyOIML2008} and \emph{WELMEC \DIFdelbegin \footnote{\DIFdel{WELMEC is the European committee to promote cooperation in the field of legal metrology.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend Software Guide 7.2} \cite{EuropeanCooperationinLegalMetrologyWELMEC2015} are probably the more widespread standards for \DIFdelbegin \DIFdel{software controlled }\DIFdelend \DIFaddbegin \DIFadd{software-controlled }\DIFaddend MI design, deployment and inspection.
...
%DIF > \footnote{OIML is the International Organization of Legal Metrology.} 
%DIF > \footnote{WELMEC is the European committee to promote cooperation in the field of legal metrology.}
\DIFaddbegin 

\DIFaddend The majority of security issues with MI \DIFdelbegin \DIFdel{are associated to economic undue advantages.
}\DIFdelend \DIFaddbegin \DIFadd{arise from parties seeking undue economic advantages.
%DIF > are associated to economic undue advantages.
}\DIFaddend %taken by attackers, which are interested in the measured physical quantity value.
...
Measurement frauds against MI (such as scales, energy meters and fuel pumps) are very common in developing countries \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Luchsinger2008,Camara2012}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Camara2012,Luchsinger2008}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
Attacks can also intend to steal sensitive information and intellectual property \cite{Camara2012,Oppermann2018}.
In some cases they even \DIFdelbegin \DIFdel{threat people}\DIFdelend \DIFaddbegin \DIFadd{threaten people's }\DIFaddend physical integrity (e.g., tampering measurements related to medical procedures) \cite{Boccardo2014}.

\subsection{Distributed measuring}
Distributed measuring (DM)\DIFdelbegin \DIFdel{is a concept supported by previous works and correlated ideas.
}\DIFdelend \DIFaddbegin \DIFadd{, where components are connected through a network, is well studied.
%DIF > concept supported by previous works and correlated ideas.
}\DIFaddend Boccardo et al. \cite{Boccardo2014} describes a strategy to simplify MI type approval and supervision activities related to medical MI.
...
Although it is not a DM case, \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend approach suggests that part of the measurement computing can be done externally to the sphygmomanometer hardware core due to the use of a digital signature to check sensing data integrity and authenticity.
\DIFdelbegin \DIFdel{In turn, }\DIFdelend Peters et al. \cite{Peters2015} describes a MI security framework using virtual machines to \DIFdelbegin \DIFdel{get separation among }\DIFdelend \DIFaddbegin \DIFadd{separate }\DIFaddend \emph{legally relevant} (LR) and \emph{non-legally relevant} (NLR) software.\footnote{OIML D 31(E) and WELMEC 7.2 use LR to designate any component which can affect measuring final results, while NLR cannot do that.} The authors propose different virtual machines to execute LR and NLR functions and define secure interfaces for communicating among them. This approach is presented as an alternative to improve security and reduce MI complexity. Additionally, it enables virtualization using different hardware cores and consequently allows the implementation of \emph{Distributed Measuring Systems} (DMS). Lastly, a DM architecture using cloud computing is discussed by Oppermann et al. \cite{Oppermann2018}. The authors present advantages related to IT infrastructure cost-savings and the possibility of MI manufacturers to offer modern interconnected devices and features.
They also present a comprehensive example of how \DIFdelbegin \DIFdel{integrates }\DIFdelend \DIFaddbegin \DIFadd{to integrate }\DIFaddend energy meters in a DMS. 
...
Blockchain is an emerging technology which has \DIFdelbegin \DIFdel{called }\DIFdelend \DIFaddbegin \DIFadd{caught }\DIFaddend the attention of stakeholders in different industry segments. 
Initially associated with crypto-currency markets due to Bitcoin popularity \cite{Nakamoto2008}, blockchain-based architectures have been proposed for a wide set of application areas\DIFaddbegin \DIFadd{, }\DIFaddend including sensor networks, internet of things, smart cities, among others \cite{Christidis2016}. 
...
Blockchain platforms can be classified as \emph{permissionless}, \DIFdelbegin \DIFdel{when }\DIFdelend \DIFaddbegin \DIFadd{in which }\DIFaddend anybody can join and participate in the network consensus, or \emph{permissioned}, \DIFdelbegin \DIFdel{when }\DIFdelend \DIFaddbegin \DIFadd{in which }\DIFaddend consensus is achieved by a set of known and identifiable peers \cite{Vukolic2016}. Usually, permissioned blockchains consensus protocols expend less computational resources and can reach better transaction latency and throughput.
%\cite{Sousa2018}.
...
This makes \DIFdelbegin \DIFdel{blockchain }\DIFdelend \DIFaddbegin \DIFadd{blockchains }\DIFaddend not only a data storage solution but also a complete distributed platform for proper and distributed automated workflow \cite{Christidis2016}.
...
In this work, we want to evaluate the security level of different measuring systems models and \DIFdelbegin \DIFdel{so }\DIFdelend point out the advantages and drawbacks of \DIFdelbegin \DIFdel{each model}\DIFdelend \DIFaddbegin \DIFadd{these models}\DIFaddend . 
We are especially interested in MI reliability and the required \DIFdelbegin \DIFdel{efforts }\DIFdelend \DIFaddbegin \DIFadd{effort }\DIFaddend for providing MI metrological assurance.
Metrological requirements and activities are very particular for different MI classes. 
However, a simple set of requirements and activities are representative \DIFdelbegin \DIFdel{to a most of }\DIFdelend \DIFaddbegin \DIFadd{of most }\DIFaddend software-controlled MI concerning security properties. 
...
\label{s:attackmodel}
We consider a simple attack model that can be built from MI use cases, \DIFdelbegin \DIFdel{accordingly }\DIFdelend \DIFaddbegin \DIFadd{according }\DIFaddend to OIML D 31(E) and WELMEC 7.2 guides. MI are targeted by malicious entities trying to get undue \DIFdelbegin \DIFdel{economical }\DIFdelend \DIFaddbegin \DIFadd{economic }\DIFaddend advantages by tampering with measurements. 
Basically, the attacker capability consists \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend changing the MI expected behavior, tampering any LR component and compromising \DIFdelbegin \DIFdel{measurements reliability }\DIFdelend \DIFaddbegin \DIFadd{the reliability of the measurements}\DIFaddend .
...
Although LR and NLR software separation is a well-known concept, many MI manufacturers do not adopt such \DIFaddbegin \DIFadd{a }\DIFaddend practice.
The claimed reasons are costs, computational \DIFdelbegin \DIFdel{resources }\DIFdelend \DIFaddbegin \DIFadd{resource }\DIFaddend restrictions or the existence of \DIFdelbegin \DIFdel{a software legacy }\DIFdelend \DIFaddbegin \DIFadd{legacy software}\DIFaddend . 
...
\item Software validation and verification can become more difficult due to the diversity of MI use cases, \DIFdelbegin \DIFdel{being }\DIFdelend many of them \DIFaddbegin \DIFadd{being }\DIFaddend hard to reproduce out of the real measurement environment. 
\item Metrological supervision requires notified bodies to have sufficient staff to proceed with MI surveillance activities in both manufacturing and the field. Although physical seals can be helpful to protect physical components, they are \DIFdelbegin \DIFdel{innocuous to protect }\DIFdelend \DIFaddbegin \DIFadd{ineffective for protecting }\DIFaddend software components.
...
In any case, a significant amount of software is moved from the MI to the cloud, \DIFdelbegin \DIFdel{and it works }\DIFdelend \DIFaddbegin \DIFadd{being provided }\DIFaddend as a service. 
Consequently, LR and NLR software can be scaled accordingly to the demand.

A DMS \DIFdelbegin \DIFdel{also enables easily }\DIFdelend \DIFaddbegin \DIFadd{easily enables }\DIFaddend software separation.
This happens because LR and NLR software do not run over a monolithic platform anymore.
That encourages manufacturers to \DIFdelbegin \DIFdel{eliminate coupling between }\DIFdelend \DIFaddbegin \DIFadd{decouple }\DIFaddend LR and NLR software, which consequently makes LR software less complex.
...
\begin{table*}[t]
\centering
\caption{Traditional MI, cloud model and blockchain model security analisys summary\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
\textbf{A2}                       & Necessary                                    & Necessary, but LR software use cases are reduced and its V\&V can be performed without the need of field tests.               & Necessary, but LR software use cases are reduced and its V\&V can be performed without the need of field tests.      \\ \midrule
...
Also, the blockchain \DIFdelbegin \DIFdel{also }\DIFdelend would support the execution of LR (and even NLR) software using smart contracts, which process information from sensors and generate a consolidated measurement value. 
The integrity of measurements and LR software (as smart contracts) is preserved by \DIFaddbegin \DIFadd{the }\DIFaddend blockchain inherent properties \cite{Zheng2017}.
...
Thus, one can expect that a blockchain model will require the contribution of different parties interested in the measurement activities, and consequently it \DIFaddbegin \DIFadd{will }\DIFaddend need to be \DIFdelbegin \DIFdel{idealized }\DIFdelend \DIFaddbegin \DIFadd{designed }\DIFaddend following a different philosophy.
...
However, they are coded by different parties that do not have conflicts of \DIFdelbegin \DIFdel{interests }\DIFdelend \DIFaddbegin \DIFadd{interest }\DIFaddend related to the measured quantity.
Whatever the measurement result \DIFaddbegin \DIFadd{is}\DIFaddend , these parties shall be rewarded by a \DIFdelbegin \DIFdel{preset }\DIFdelend \DIFaddbegin \DIFadd{pre-set }\DIFaddend value. 
...
This concept also breaks \DIFdelbegin \DIFdel{with the manner MI are traditionally }\DIFdelend \DIFaddbegin \DIFadd{the traditional way MI are }\DIFaddend used in consumer relations\DIFdelbegin \DIFdel{.
Furthermore, it creates }\DIFdelend \DIFaddbegin \DIFadd{, creating }\DIFaddend a new market for players who want to offer computing services for measuring. 
...
However, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend more organizations take part in the consensus, more expensive and unfeasible these attacks become.
...
Initially, we evaluate \DIFdelbegin \DIFdel{the }\DIFdelend traditional MI security. 
In this scenario, one should note that BMAF requirements and activities are necessary to prevent attacks.
As already discussed at Section \ref{s:mi_traditional}, the \DIFdelbegin \DIFdel{efforts required for proceeding with traditional MI }\DIFdelend \DIFaddbegin \DIFadd{activities of }\DIFaddend control and supervision \DIFdelbegin \DIFdel{activities are distinguished}\DIFdelend \DIFaddbegin \DIFadd{of traditional MI involve a substantial effort}\DIFaddend .
Documentation analysis, code inspection, and software validation and verification need to \DIFdelbegin \DIFdel{comprise }\DIFdelend \DIFaddbegin \DIFadd{be done on }\DIFaddend all components and software modules.
...
In A1, the document analysis and the code inspection of LR software running in the cloud are expected to require \DIFdelbegin \DIFdel{fewer efforts }\DIFdelend \DIFaddbegin \DIFadd{less effort }\DIFaddend than embedded software evaluation. 
...
\DIFdelbegin \DIFdel{In turn}\DIFdelend \DIFaddbegin \DIFadd{Similarly}\DIFaddend , A3 also becomes less expensive because LR software identification and integrity check are executed against cloud servers, which are far fewer than the deployed MI. 
Finally, field surveillance for checking MI physical seals can also be eliminated. 
If we assume simplified MI as immutable instruments, they can be conceived as \DIFdelbegin \DIFdel{tamper proofing }\DIFdelend \DIFaddbegin \DIFadd{tamper-proof }\DIFaddend devices. 
...
Regarding the activities, although A1 and A2 are still necessary, they should become much simpler when compared to the \DIFdelbegin \DIFdel{required effort in the }\DIFdelend other models.
\DIFdelbegin \DIFdel{That is }\DIFdelend \DIFaddbegin \DIFadd{This happens }\DIFaddend because the structure of smart contracts \DIFdelbegin \DIFdel{limits significantly }\DIFdelend \DIFaddbegin \DIFadd{significantly limits }\DIFaddend the complexity resulting from having different technologies, software components and programming languages while imposing software separation.
Finally, A3 becomes unnecessary in a blockchain network \DIFdelbegin \DIFdel{due to }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the same reasons as R2.
...
Once LR software is produced by players who are exempted from \DIFdelbegin \DIFdel{conflict of interests}\DIFdelend \DIFaddbegin \DIFadd{conflicts of interest}\DIFaddend , many activities related to the assurance of software correctness and integrity are made simpler or even unnecessary. 
...
In this section, we develop an experiment to demonstrate \DIFdelbegin \DIFdel{our proposal feasibility}\DIFdelend \DIFaddbegin \DIFadd{the feasibility of our proposal}\DIFaddend .
...
Furthermore, over the last two years, we have proceeded with formal type approval of speed meters in the Brazilian National Institute of Metrology, Quality and Technology (Inmetro\DIFdelbegin \DIFdel{)}\DIFdelend \footnote{\url{http://www.inmetro.gov.br}}\DIFaddbegin \DIFadd{)}\DIFaddend .
This experience provides valuable information that supports \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend analysis in this section.  

Developing countries are essentially guided for legal metrology policies related to fraud detection and avoidance \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Luchsinger2008,Camara2012,RodriguesFilho2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{RodriguesFilho2015,Camara2012,Luchsinger2008}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
In this aspect, the blockchain-based DMS can introduce promising advantages\DIFdelbegin \DIFdel{.
It }\DIFdelend \DIFaddbegin \DIFadd{, as it }\DIFaddend simplifies the metrological assurance framework activities, \DIFdelbegin \DIFdel{at the same time that keeps }\DIFdelend \DIFaddbegin \DIFadd{while preserving }\DIFaddend many of the advantages from a DMS \DIFdelbegin \DIFdel{concerning }\DIFdelend \DIFaddbegin \DIFadd{in terms of }\DIFaddend performance and costs saving.
...
Besides, speed meters manufacturers complain about opening their \DIFdelbegin \DIFdel{complete solution }\DIFdelend \DIFaddbegin \DIFadd{solutions }\DIFaddend for inspection due to intellectual property issues.
That is the case of the speed meters evaluated \DIFdelbegin \DIFdel{for us }\DIFdelend in Inmetro.
So they are representative cases of the traditional MI model described in this work.

Another aspect is that speed \DIFdelbegin \DIFdel{meters }\DIFdelend \DIFaddbegin \DIFadd{meter }\DIFaddend owners usually deploy their equipment at far places along roads spread over large geographic areas.
...
Thus, all those aspects make speed meters strong candidates for solutions that help to separate LR and NLR software\DIFdelbegin \DIFdel{, while offering easiness for legal metrology activities such as documentation analysis, code inspection, V\&V, and field surveillance.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > , while offering efficiencies for legal metrology activities such as documentation analysis, code inspection, V\&V, and field surveillance.
}\DIFaddend 

We implement such \DIFaddbegin \DIFadd{a }\DIFaddend solution creating a blockchain network for distributed measuring.
We \DIFdelbegin \DIFdel{condense }\DIFdelend \DIFaddbegin \DIFadd{integrate }\DIFaddend the speed meter's LR features in a simple \DIFdelbegin \DIFdel{tamper proofing }\DIFdelend \DIFaddbegin \DIFadd{tamper-proof }\DIFaddend hardware that uses two inductive sensors to capture the vehicle's magnetic profile \cite{S2011}.
...
\DIFdelbegin \DIFdel{In turn, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend blockchain executes the LR software as a smart contract and computes the vehicle speed.
...
\caption{The blockchain-based vehicle speed DMS\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
\label{f:speedmeter}
\end{figure}
...
Endorsers are peers that effectively execute smart contracts, which are called \DIFdelbegin \DIFdel{chaincodes }\DIFdelend \DIFaddbegin \emph{\DIFadd{chaincodes}} \DIFaddend in Fabric.
...
\DIFdelbegin \DIFdel{In turn, security }\DIFdelend \DIFaddbegin \DIFadd{Security }\DIFaddend policies can set the way the network validates the measurement provided by endorsers.
\DIFdelbegin \DIFdel{Security }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend policies can also work as a protection mechanism against collusion attacks and implement metrological surveillance.
...
The more organizations are involved, the more expensive it becomes \DIFaddbegin \DIFadd{to carry out }\DIFaddend a collusion attack.
Such monitoring activities are a sort of metrological surveillance since different organizations are continually checking the measurements resulting from \DIFaddbegin \DIFadd{a }\DIFaddend chaincode execution.

Since we resort to a permissioned blockchain, the consensus \DIFaddbegin \DIFadd{protocol }\DIFaddend plays an important role in our experiment.
...
We set up our vehicle speed DMS \DIFdelbegin \DIFdel{according to the scheme }\DIFdelend \DIFaddbegin \DIFadd{as illustrated }\DIFaddend in Figure~\ref{f:scheme}.
...
\DIFdelbegin \DIFdel{An idea consists in }\DIFdelend \DIFaddbegin \DIFadd{One idea consists of }\DIFaddend encrypting the images using asymmetric cryptography before sending them to the blockchain.
...
To do that, the notified body inserts into the blockchain a new transaction comprised by the chaincode \DIFdelbegin \DIFdel{'s }\DIFdelend fingerprint (i.e., the software image hash) and its respective security policy.
...
After \DIFdelbegin \DIFdel{instantiated}\DIFdelend \DIFaddbegin \DIFadd{instantiation}\DIFaddend , the chaincode fingerprint becomes immutable, following the intrinsic blockchain properties.
...
\caption{Vehicle speed DMS solution scheme\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
\label{f:scheme}
\end{figure}
...
These countermeasures match \DIFdelbegin \DIFdel{with }\DIFdelend the properties already discussed \DIFdelbegin \DIFdel{at the Security Analysis.
We }\DIFdelend \DIFaddbegin \DIFadd{in Section~\ref{s:sec_analysis}.
%DIF > We describe some attacks and how the implementation using a simple MI conception and Fabric blockchain addresses them.
In the following we }\DIFaddend describe some attacks and how \DIFdelbegin \DIFdel{the implementation using a simple MI conception and Fabric blockchain addresses }\DIFdelend \DIFaddbegin \DIFadd{our blockchain-based DMS deals with }\DIFaddend them.
...
Consequently, any attempt at violating seals or stealing a private key shall \DIFdelbegin \DIFdel{flaw }\DIFdelend \DIFaddbegin \DIFadd{destroy }\DIFaddend the speed meter hardware, forcing its replacement and exposing the attacker.
...
For instance, suppose that an attacker \DIFdelbegin \DIFdel{gets collusion }\DIFdelend \DIFaddbegin \DIFadd{colludes }\DIFaddend with LaSIGE, \DIFdelbegin \DIFdel{obtaining compromised peers that }\DIFdelend \DIFaddbegin \DIFadd{making compromised peers }\DIFaddend accept a modified LR chaincode.
...
The attacker needs to compromise more organizations, which makes the attack too expensive and\DIFdelbegin \DIFdel{consequently }\DIFdelend \DIFaddbegin \DIFadd{, consequently, }\DIFaddend unfeasible.
...
Assuming such demand for a total of 1 thousand speed meters, we need a blockchain network able to process something around 800 \DIFdelbegin \DIFdel{tsp }\DIFdelend \DIFaddbegin \DIFadd{tps }\DIFaddend (transactions per second).
Androulaki et al. \cite{Androulaki2018} benchmarks Fabric performance in something between 2,000 to 3,000 \DIFdelbegin \DIFdel{tsp}\DIFdelend \DIFaddbegin \DIFadd{tps}\DIFaddend .
...
The transactions simulation uses real data from speed meters developed by the company Perkons\DIFdelbegin \DIFdel{SA}\DIFdelend \footnote{\url{http://www.perkons.com}} \DIFaddbegin \DIFadd{SA}\DIFaddend , which kindly granted a dataset for this experiment.

\begin{figure}[!t]
\centering
\includegraphics[width=.45\textwidth]{chartsolo} %[width=2.5in]
\caption{Throughput and latency using native Fabric solo orderer\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
\label{f:latth}
\end{figure}
...
When a client \DIFdelbegin \DIFdel{process }\DIFdelend instance is created, it selects a vector of bytes containing the sensors raw data from the dataset mentioned above, for each transaction.
...
With \DIFaddbegin \DIFadd{the }\DIFaddend solo orderer service, throughput and latency reach a better trade-off around 300 tps and 1 second, respectively.
...
\DIFdelbegin \DIFdel{A strategy of queuing transactions }\DIFdelend \DIFaddbegin \DIFadd{This high latency can be explained by the transactions queuing }\DIFaddend in the solo orderer\DIFdelbegin \DIFdel{can explain the high latency.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > A strategy of queuing transactions in the solo orderer can explain the high latency.
}\DIFaddend %However, this method compromises performance, resulting in low throughput and misusing of the MI communication channel.

\DIFdelbegin \DIFdel{In turn, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend BFT orderer service with four replicas performs better.
...
The BFT orderer also keeps throughput \DIFdelbegin \DIFdel{in a steady state }\DIFdelend \DIFaddbegin \DIFadd{stable at }\DIFaddend about 360 tps even with a workload of 1,200 clients, presenting only a slight increase \DIFdelbegin \DIFdel{in latency }\DIFdelend of 1 second \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{in latency.
The }\DIFaddend BFT orderer optimizes latency by discarding the exceeding of transactions after reaching \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend max throughput.
However, clients need to control refused transactions, creating \DIFdelbegin \DIFdel{an own queue and trying to send }\DIFdelend \DIFaddbegin \DIFadd{their queue and resending }\DIFaddend the transaction again after some time.
...
\DIFadd{The }\DIFaddend BFT orderer service also includes an important aspect.
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=.45\textwidth]{chartbft4r} %%%
%DIF < [width=2.5in]
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Throughput and latency using BFT orderer with four replicas}}
%DIFAUXCMD
%DIFDELCMD < \label{f:latthbft4}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Quantitative Assessment}
...
In our analysis, we adopt the average ratio of approximately 6 D/\DIFdelbegin \DIFdel{KLOC }\DIFdelend \DIFaddbegin \DIFadd{kLOC }\DIFaddend (defects by thousands of lines of code) reported by Carrozza et al. \cite{Carrozza2015}.
...
This software size statistically suggests the \DIFdelbegin \DIFdel{remaining }\DIFdelend \DIFaddbegin \DIFadd{existence }\DIFaddend of about 18 software defects and, consequently, a high probability of \DIFdelbegin \DIFdel{the existence of potential }\DIFdelend \DIFaddbegin \DIFadd{having a }\DIFaddend vulnerability.
Such LR software size is a consequence of the strong coupling \DIFdelbegin \DIFdel{which imposes dependences among }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend LR and NLR modules.
...
Its LR software consists of a Go language \DIFdelbegin \DIFdel{smart contract }\DIFdelend \DIFaddbegin \DIFadd{chaincode }\DIFaddend that requires no more than 200 LOCs.
Such size points out an \DIFdelbegin \DIFdel{estimative }\DIFdelend \DIFaddbegin \DIFadd{estimate }\DIFaddend of no more than two remaining software defects, which also reduces the chances of potential vulnerabilities.

\begin{table}[t!]
\centering
\caption{Advantages and drawbacks of the Traditional MI and our Fabric DMS implementation\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
Expected software defects (D/\DIFdelbeginFL \DIFdelFL{KLOC}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{kLOC}\DIFaddendFL ) & 18                      & 2                   \\ \midrule
...
Since the majority of the manufacturers implement their software in C language, we adopt the FP/\DIFdelbegin \DIFdel{KLOC }\DIFdelend \DIFaddbegin \DIFadd{kLOC }\DIFaddend conversion rate in the QSM Function Point Languages Table \footnote{http://www.qsm.com/resources/function-point-languages-table} of approximately 100 LOCs per FP.
...
However, we understand that the NLR software \DIFdelbegin \DIFdel{also can }\DIFdelend \DIFaddbegin \DIFadd{can also }\DIFaddend be provided by independent remote services.
...
Concerning the demand by connectivity, we can \DIFdelbegin \DIFdel{stat }\DIFdelend \DIFaddbegin \DIFadd{state }\DIFaddend that both solutions are similar.
Although the traditional MI can send information in batch mode, the expected \DIFdelbegin \DIFdel{information amount }\DIFdelend \DIFaddbegin \DIFadd{amount of information }\DIFaddend is practically the same \DIFdelbegin \DIFdel{than }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend in the blockchain-based DMS.
\DIFdelbegin \DIFdel{In turn}\DIFdelend \DIFaddbegin \DIFadd{Furthermore}\DIFaddend , despite the extensive use of cryptography in a blockchain application, that does not represent a \DIFdelbegin \DIFdel{significative }\DIFdelend \DIFaddbegin \DIFadd{significant }\DIFaddend overhead regarding the amount of propagated information.
Roughly speaking, the use of connectivity resources depends on the number of detected vehicles and not from the adopted \DIFdelbegin \DIFdel{solution }\DIFdelend model.
...
So we can \DIFdelbegin \DIFdel{stat }\DIFdelend \DIFaddbegin \DIFadd{state }\DIFaddend that the $MTBF_h$ of the simple MI hardware used int the blockchain-based DMS is expected to be higher than the $MTBF_H$ of traditional MI.
...
However, we can \DIFdelbegin \DIFdel{stat }\DIFdelend \DIFaddbegin \DIFadd{state }\DIFaddend two crucial aspects:
\begin{itemize}
 ...
 \item The blockchain-based DMS is expected to present a better availability ratio due to its \DIFdelbegin \DIFdel{replicas that provide fault toleranceto the system}\DIFdelend \DIFaddbegin \DIFadd{inherent fault tolerance}\DIFaddend .
\end{itemize}
...
\textbf{$\bullet$ The measurement Big Data:} MI usually manipulate a high amount of data. In a \DIFdelbegin \DIFdel{large scale }\DIFdelend \DIFaddbegin \DIFadd{large-scale }\DIFaddend scenario (e.g., energy meters in a smart grid), MI can update their measurements faster, generating lots of transactions. 
A network connecting millions of meters may generate a transaction load unfeasible to be processed by existing blockchains\DIFdelbegin \DIFdel{implementations}\DIFdelend . 
...
Besides, one can try to use aggregated measurements for reducing transactions in a blockchain\DIFdelbegin \DIFdel{implementation}\DIFdelend . 
...
Thus blockchain-based measuring is not \DIFdelbegin \DIFdel{proper }\DIFdelend \DIFaddbegin \DIFadd{appropriate }\DIFaddend for all MI applications. 
Furthermore, attacks \DIFdelbegin \DIFdel{aiming }\DIFdelend \DIFaddbegin \DIFadd{targeting }\DIFaddend communication (e.g., DDoS) represent an additional risk, although decentralized systems such as blockchains are more resilient to such attacks than conventional cloud architectures.
...
\section{Conclusion}
In this paper \DIFdelbegin \DIFdel{, we discussed a way }\DIFdelend \DIFaddbegin \DIFadd{we discussed how }\DIFaddend blockchains can be used to support DMS.
...
Our results were consistent, and they support \DIFdelbegin \DIFdel{our idea feasibility }\DIFdelend \DIFaddbegin \DIFadd{the feasibility of our proposal}\DIFaddend . 
...
Future \DIFdelbegin \DIFdel{works }\DIFdelend \DIFaddbegin \DIFadd{work }\DIFaddend shall include a complete risk analysis of our blockchain-based model \DIFdelbegin \DIFdel{, as so as }\DIFdelend \DIFaddbegin \DIFadd{to develop }\DIFaddend new strategies for addressing the challenges discussed \DIFdelbegin \DIFdel{at the end}\DIFdelend \DIFaddbegin \DIFadd{here}\DIFaddend .
