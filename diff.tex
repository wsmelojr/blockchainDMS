...
%DIF DEL bare_conf.tex   Sat Jul 14 09:12:52 2018
%DIF ADD tim.tex         Sat Jul 14 15:58:07 2018
%DIF < %% bare_conf.tex
%DIF -------
 %DIF > 
%% bare_jrnl.tex %DIF > 
...
\subsection{\DIFadd{Implementing vehicle speed meters a distributed measuring system}}
...
Vehicle speed meters are efficient solutions for estimating the use of public roads, generating traffic statistics and enforcing speed limits for drivers.
...
Besides, speed meters manufacturers complain about opening their complete solution for inspection, once such practice can put at risk some industrial secret.
...
We glimpse the speed meter LR features condensed in a simple tamper proofing hardware that uses two inductive sensors for capture the vehicle's magnetic profile.
After detecting a vehicle, this hardware uses a private key to sign the sensor data raw and sends that to a blockchain-based DMS.
...
\DIFadd{Our experiment uses Hyperledger Fabric (HLF) \mbox{%DIFAUXCMD
...
HLF is a blockchain open source platform that includes two concepts which are very helpful for implementing our idea: endorsers and security policies.
}

\DIFadd{Endorsers are peers which effectively execute smart contracts, which are called chaincodes in HLF.
...
Blockchain networks are constituted for peers that belong to different stakeholders or organizations.
...
They can specify, for instance, which organizations must take part in a transaction endorsement.
...
\DIFadd{We set up our vehicle speed meter DMS as follows.
...
HLF supports chaincode written in Go programming language.
Figure X shows a simple LR algorithm for calculating speed example in Go.
This algorithm analyses the data raw from both sensors and finds the moment when the vehicle activates each sensor.
...
\DIFadd{Our experiment supposes the implementation of legal metrology activities as follows.
Firstly, the notified body proceeds with the MI type approval and V\&V.
He does that by evaluating the MI device (i.e., inspecting sensors, cryptographic and communication features) and the LR source code (i.e., the HLF chaincode).
...
\DIFadd{In HLF, a chaincode instantiation means to notify the endorsers that they may execute that chaincode.
...
Once the MI owner has available endorser peers, he needs to install the approved LR chaincode in each one of them.
That enables any MI in the field to generate transactions, ask endorser peers for determining the vehicle speed,  and so store the vehicle speed legally relevant information in the blockchain ledger.
}
...
Essentially, we are interested in two main aspects:
   ...
   \item{The throughput of a simple blockchain network configuration. We test a high number of transactions against a network consisting of only two peers (one of them as an endorser) and a (BFT???) consensus service.}
 \end{itemize} 
...
Sao Paulo is a big city with a vehicular fleet above 8 million vehicles.
...
\DIFadd{Two works benchmark HLF performance in something around 2,000 and 3,000 tsp \mbox{%DIFAUXCMD
\cite{Sousa2017,Androulaki2018}}%DIFAUXCMD
.
...
We use HLF just as provided by its developers, using an ordinary hardware infrastructure available in any datacenter.
That is purposeful to verify the real results that a speed meter solution owner can obtain by using HLF.
}
...
HLF standard distribution runs over }\emph{\DIFadd{docker containers}} \DIFadd{so that each physical node can host several peers.
For practical reasons, we use three nodes for allocating the orderer service, LaSIGE peers, and Inmetro peers respectively.
The client transactions load is simulated using two nodes from a Dell PowerEdge PowerPowerCluster.
...
Each physical machines creates an HLF docker container client used for simulating transactions.
...
Clients use HLF protocol to invoke an LR chaincode and send such data as an argument to an endorser peer from LaSIGE organization.
...
\begin{figure}[!t]
\centering
\includegraphics[width=.45\textwidth]{graph1} %DIF > [width=2.5in]
\caption{\DIFaddFL{Throughput and latency resulting from different load levels.}}
\label{f:latth}
\end{figure}
...
In fact, there are some difficulties in effectively generating this number of concurrent transactions without to modify HLF implementation.
Since we use HLF as an off-the-shelf solution, we simulate each transaction as an independent process, which requires massive hardware resources.
