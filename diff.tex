...
%DIF ADD tim.tex         Wed Jul 18 20:03:38 2018
%DIF < %% bare_conf.tex
%DIF -------
 %DIF > 
%% bare_jrnl.tex %DIF > 
...
\DIFaddbegin \section{\DIFadd{Practical Experiment}}

\subsection{\DIFadd{Implementing vehicle speed meters as a distributed measuring system}}

\begin{figure}[!t]
\centering
\includegraphics[width=.5\textwidth]{speedmeter} %DIF > [width=2.5in]
...
\cite{EuropeanCooperationinLegalMetrologyWELMEC2015} }%DIFAUXCMD
defines this classification as instruments which run their software in a universal computer hardware.
...
\begin{figure}[!t]
\centering
\includegraphics[width=.5\textwidth]{hlfscheme} %DIF > [width=2.5in]
...
\DIFadd{We verify how our implementation offers countermeasures against typical attacks related to MI.
...
\DIFadd{One should remember that, in the proposed DMS, organizations computes measurements as an independent service (i.e., they do not have any advantage or rewarded regarding the measurement result).
...
\subsection{\DIFadd{Performance issues}}
\DIFadd{The last step in our experiment is to evaluate the speed meter DMS performance.
...
\DIFadd{Firstly, we estimate the experiment demand based on vehicles traffic real data.
...
\DIFadd{Two works benchmark Fabric performance in something around 2,000 and 3,000 tsp \mbox{%DIFAUXCMD
\cite{Androulaki2018}}%DIFAUXCMD
.
...
\begin{figure}[!t]
\centering
\includegraphics[width=.45\textwidth]{chartbft1r} %DIF > [width=2.5in]
\caption{\DIFaddFL{Throughput and latency using BFT orderer with one replica}}
\label{f:latthbft1}
\end{figure}
...
\DIFadd{Figure \ref{f:latth} depicts our tests result.
...
\DIFaddend \section{Challenges Ahead}
