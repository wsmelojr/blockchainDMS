...
%DIF DEL tim-b1.tex   Fri Jul 20 17:09:10 2018
%DIF ADD tim.tex      Mon Jul 23 15:33:11 2018
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
\thanks{W. Melo Jr. and L.F.R.C. Carmo are with the National Institute of Metrology, Quality and Technology, Duque de Caxias, RJ, Brazil; and with the Federal University of Rio de Janeiro, Brazil.}% <-this % stops a space
\DIFdelbegin %DIFDELCMD < \thanks{A. Bessani and N. Neves are with LaSIGE, Faculdade de Ciências, Universidade de Lisboa, Portugal.}%%%
\DIFdelend \DIFaddbegin \thanks{A. Bessani and N. Neves are with LaSIGE, Faculdade de Ci\^encias, Universidade de Lisboa, Portugal.}\DIFaddend % <-this % stops a space
...
In this \DIFdelbegin \DIFdel{work}\DIFdelend \DIFaddbegin \DIFadd{paper}\DIFaddend , we evaluate \DIFdelbegin \DIFdel{how blockchains can help }\DIFdelend \DIFaddbegin \DIFadd{the use of blockchains as a resource }\DIFaddend to overcome such challenges.
...
We evaluate the security and performance of our blockchain-based measuring system by executing tests with data from real speed meters sensors.
The results are promising and consolidate the feasibility of our idea.
...
Measurement instruments (MI) are used in a diversity of applications including industry, commerce, energy, transportation, medical care and environment protection \cite{RodriguesFilho2015}. 
...
In turn, metrological supervision becomes difficult due to the high number of MI different models and versions, the capillarity of their deployment and the limited resources owned by regulation agencies.

We work with the hypothesis that \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{the aforementioned }\DIFaddend difficulties should be overcome with alternative approaches that simplify MI design while employing strategies for decentralizing metrological supervision. 
Such idea finds many aspects in common with a new trendy technology: \emph{blockchains} \cite{Zheng2017}. 
...
More recently, a few works have proposed blockchain applications in legal metrology\DIFaddbegin \DIFadd{, }\DIFaddend which include decentralized audit, mechanisms for software loading, PKI for MI manufacturers and Distributed Measuring Systems (DMS) \cite{Peters2018,MeloJr.2017}.
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend paper extends the ideas presented in our previous work \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{under the title ``How Blockchains can improve Measuring Instruments Regulation and Control'' }\DIFaddend \cite{MeloJr.2018a}. 
We start from preliminary concepts already consolidated in \DIFaddbegin \DIFadd{Legal Metrology about }\DIFaddend MI regulation and control. 
...
Our main contributions can be summarized as follows:
 ...
 \item We develop a practical case study using \DIFdelbegin \DIFdel{HyperLedger Fabric framework}\DIFdelend \DIFaddbegin \DIFadd{the Hyperledger Fabric \mbox{%DIFAUXCMD
\cite{Androulaki2018} }%DIFAUXCMD
platform}\DIFaddend . We create a blockchain network \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend implements legally relevant software using smart contracts for measuring vehicle speed. We also present \DIFdelbegin \DIFdel{consistent }\DIFdelend results that demonstrate \DIFdelbegin \DIFdel{our idea feasibility}\DIFdelend \DIFaddbegin \DIFadd{the feasibility of our idea}\DIFaddend .
...
A communication interface ables the }\DIFaddend MI to send sensing raw data to the cloud measuring system. 
...
\section{\DIFdelbegin \DIFdel{Practical Experiment}\DIFdelend \DIFaddbegin \DIFadd{Case Study}\DIFaddend }
...
We develop \DIFdelbegin \DIFdel{a practical }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend experiment to demonstrate our proposal feasibility.
It consists of a vehicle speed DMS using blockchains (Figure \ref{f:speedmeter}).
\DIFdelbegin \DIFdel{Vehicle speed }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend meters are efficient solutions for estimating \DIFdelbegin \DIFdel{the use of }\DIFdelend \DIFaddbegin \DIFadd{vehicle speed on }\DIFaddend public roads, generating traffic statistics and enforcing speed limits for drivers \cite{Ki2006,S2011}.
The meter detects each vehicle, \DIFdelbegin \DIFdel{determine }\DIFdelend \DIFaddbegin \DIFadd{determines }\DIFaddend its speed and \DIFdelbegin \DIFdel{capture }\DIFdelend \DIFaddbegin \DIFadd{captures }\DIFaddend one or more pictures identifying the vehicle's license plate when necessary.
...
WELMEC 7.2 \cite{EuropeanCooperationinLegalMetrologyWELMEC2015} defines this classification as instruments \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend run their software in universal computer hardware.
...
Another aspect is that MI owners usually deploy their speed meters \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend far places along roads spread \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{over }\DIFaddend large geographic areas.
That \DIFdelbegin \DIFdel{can difficult }\DIFdelend \DIFaddbegin \DIFadd{increases the difficulty of }\DIFaddend regular inspection activities and consequently increases costs associated with metrological surveillance.
Thus\DIFaddbegin \DIFadd{, }\DIFaddend all those aspects make speed meters strong candidates for solutions that help to separate LR and NLR software\DIFaddbegin \DIFadd{, }\DIFaddend while offering easiness for legal metrology activities such as type approval, V\&V, and field surveillance.

We implement such solution creating a blockchain network for distributed measuring.
We \DIFdelbegin \DIFdel{collect }\DIFdelend \DIFaddbegin \DIFadd{condense }\DIFaddend the speed meter\DIFdelbegin \DIFdel{LR features condensed }\DIFdelend \DIFaddbegin \DIFadd{'s LR features }\DIFaddend in a simple tamper proofing hardware that uses two inductive sensors \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend capture the vehicle's magnetic profile \cite{S2011}.
...
In turn, the blockchain executes the LR software as a smart contract and \DIFdelbegin \DIFdel{determines }\DIFdelend \DIFaddbegin \DIFadd{computes }\DIFaddend the vehicle speed.
...
Our experiment uses Hyperledger Fabric \cite{Androulaki2018} for developing a permissioned blockchain\DIFaddbegin \DIFadd{, }\DIFaddend where the peers cooperate to store measurements and execute LR software.
Fabric is an open source blockchain platform that includes two concepts \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend are very helpful for implementing our idea: \emph{endorsers} and \emph{security policies}.

Endorsers are peers \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend effectively execute smart contracts, which are called chaincodes in Fabric.
That has significant implications concerning intellectual property and performance.
First, a manufacturer needs to reveal his LR software only \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend peers contracted to execute his measuring chaincode as a service, and to the notified body responsible for approving it.
Second, a manufacturer can size his solution performance by providing \DIFdelbegin \DIFdel{how }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend many endorser peers as necessary, resulting in a scalable architecture.
\DIFaddbegin 

\DIFaddend In turn, security policies can set \DIFdelbegin \DIFdel{how }\DIFdelend \DIFaddbegin \DIFadd{the way }\DIFaddend the network validates the measurement provided by endorsers.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In complement, security policies can }\DIFdelend \DIFaddbegin \DIFadd{Security policies can also }\DIFaddend work as a protection mechanism against collusion attacks and also implement metrological surveillance.
...
\DIFdelbegin \DIFdel{Once we propose }\DIFdelend \DIFaddbegin \DIFadd{Since we resort to }\DIFaddend a permissioned blockchain, the consensus plays an important role in our experiment.
Fabric \DIFdelbegin \DIFdel{referee consensus as }\DIFdelend \DIFaddbegin \DIFadd{refers to consensus as an }\DIFaddend \emph{orderer service}.
We perform our experiment with two different types of orderer services: the \emph{solo orderer} and the \emph{BFT orderer}.
The solo orderer service is native from Fabric distribution and \DIFaddbegin \DIFadd{it is }\DIFaddend very practical to implement tests and \DIFdelbegin \DIFdel{concept proofs}\DIFdelend \DIFaddbegin \DIFadd{make proof of concept}\DIFaddend .
...
In turn, the BFT orderer is developed by Sousa et al. \cite{Sousa2018} for tolerating Byzantine failures.
Thus\DIFaddbegin \DIFadd{, }\DIFaddend one can configure BFT orderer to work with more than one replica, with different organizations controlling each one of them.
Such \DIFdelbegin \DIFdel{an }\DIFdelend approach results in a decentralized consensus\DIFdelbegin \DIFdel{while provides }\DIFdelend \DIFaddbegin \DIFadd{, thus providing }\DIFaddend security against collusion attacks.
...
Thus\DIFaddbegin \DIFadd{, }\DIFaddend a collusion attack including only \DIFaddbegin \DIFadd{the }\DIFaddend LaSIGE organization does not succeed.
...
One should remember that, in the proposed DMS, organizations compute measurements as an independent service (i.e., they do not have any advantage or rewarded regarding the measurement result).
Such business model discourages collusion and makes these attacks \DIFdelbegin \DIFdel{less feasible}\DIFdelend \DIFaddbegin \DIFadd{disadvantageous}\DIFaddend .
Even so, \DIFdelbegin \DIFdel{supposing }\DIFdelend \DIFaddbegin \DIFadd{assuming }\DIFaddend that an attacker succeeds in compromising a \DIFdelbegin \DIFdel{sufficient }\DIFdelend \DIFaddbegin \DIFadd{sufficiently large }\DIFaddend number of peers for endorsing a malicious transaction, one can expose such fraud by auditing the measurement record.
...
We try to estimate the blockchain peers best \DIFdelbegin \DIFdel{achievement without to consider }\DIFdelend \DIFaddbegin \DIFadd{behavior without considering }\DIFaddend network communication issues.
...
The same report analyses the \DIFdelbegin \DIFdel{flux of vehicles }\DIFdelend \DIFaddbegin \DIFadd{vehicles flow }\DIFaddend on the main roads in the city and points out an average of 2,\DIFdelbegin \DIFdel{736 }\DIFdelend \DIFaddbegin \DIFadd{772 }\DIFaddend vehicles/hour (or 0.76 vehicles/sec) during \DIFdelbegin \DIFdel{the peak hours}\DIFdelend \DIFaddbegin \DIFadd{rush hour}\DIFaddend .
...
Androulaki et al. \cite{Androulaki2018} benchmarks Fabric performance in something \DIFdelbegin \DIFdel{around }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend 2,000 \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend 3,000 tsp.
...
We use Fabric just as provided by its developers, \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend an ordinary hardware infrastructure available in any datacenter.
That is \DIFdelbegin \DIFdel{purposeful to verify }\DIFdelend \DIFaddbegin \DIFadd{done on purpose to evaluate }\DIFaddend the real results that a speed meter solution owner can obtain by using Fabric.

\subsection{Test environment setup}
Our blockchain network environment consists of 3 nodes from a Dell PowerEdge R410 \DIFdelbegin \DIFdel{Cluster}\DIFdelend \DIFaddbegin \DIFadd{cluster}\DIFaddend .
Each node has two CPUs Intel® Xeon® Processor E5520 with 2.27 GHz and 32 GB of RAM.
Fabric standard distribution runs over \emph{docker containers}\DIFaddbegin \DIFadd{, }\DIFaddend so that each physical node can host several peers.
For \DIFdelbegin \DIFdel{practical reasons}\DIFdelend \DIFaddbegin \DIFadd{the sake of simplicity}\DIFaddend , we use three nodes for allocating the orderer service, \DIFaddbegin \DIFadd{the }\DIFaddend LaSIGE peers, and \DIFdelbegin \DIFdel{Inmetro peers }\DIFdelend \DIFaddbegin \DIFadd{the Inmetro peers, }\DIFaddend respectively.

Regarding the orderer service, we test two different scenarios.
The first one is the native Fabric's solo orderer service, which is provided primarily for testing\DIFdelbegin \DIFdel{and does not have a practical use.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > and does not have a practical use.
}\DIFaddend The second scenario uses the orderer service developed by Sousa et al. \cite{Sousa2018}, which tolerates Byzantine failures.
\DIFdelbegin \DIFdel{We configure this service to work with four replicas.
}\DIFdelend %DIF > We configure this service to work with four replicas.

The client transactions load is \DIFdelbegin \DIFdel{simulated }\DIFdelend \DIFaddbegin \DIFadd{mimicked }\DIFaddend using four nodes from a Dell PowerEdge R300 \DIFaddbegin \DIFadd{cluster}\DIFaddend , each one with an Intel Xeon Processor L5410 with 2.33 GHz and 8 GB of RAM.
The \DIFaddbegin \DIFadd{transactions }\DIFaddend simulation uses real data from speed meters developed by the company Perkons SA (www.perkons.com), which kindly granted a dataset for this experiment.
...
Client containers are responsible for \DIFdelbegin \DIFdel{simulating transactions.
At the test execution, each client tries to produce a workload of $n$ concurrent transactions per second.
In fact, there are some difficulties in effectively generating this number of concurrent transactionswithout modifying Fabric implementation.
Since we }\DIFdelend \DIFaddbegin \DIFadd{generating transactions.
We }\DIFaddend use Fabric as an \emph{off-the-shelf solution}, \DIFdelbegin \DIFdel{we simulate each transaction as an independent process }\DIFdelend \DIFaddbegin \DIFadd{without any customization.
Each client instance corresponds to a container process that sends transactions to the blockchain continually}\DIFaddend , which requires massive hardware resources.
\DIFdelbegin \DIFdel{Even so, the measured }\DIFdelend \DIFaddbegin \DIFadd{We try to produce a maximum workload by increasing the number of clients.
Reaching such conditions, we determine the blockchain's best }\DIFaddend throughput and latency\DIFdelbegin \DIFdel{are precise once they comprise the receiving of all simulated transactions by the orderer service.
}\DIFdelend \DIFaddbegin \DIFadd{.
...
Clients use \DIFaddbegin \DIFadd{the }\DIFaddend Fabric protocol to invoke an LR chaincode and send such data as an argument to an endorser peer from LaSIGE organization.
...
The \DIFdelbegin \DIFdel{clients also keep }\DIFdelend \DIFaddbegin \DIFadd{client also keeps }\DIFaddend records of timestamps and the elapsed time for completing the transaction.
...
The second scenario works with the BFT orderer service with four replicas\DIFdelbegin \DIFdel{, each one of them }\DIFdelend \DIFaddbegin \DIFadd{.
We consider that each replica is }\DIFaddend under control of a different organization.
The \DIFdelbegin \DIFdel{comparing }\DIFdelend \DIFaddbegin \DIFadd{comparison }\DIFaddend between these scenarios is important for evaluating performance aspects.
...
Figures \ref{f:latth} and \ref{f:latthbft4} depict our tests \DIFdelbegin \DIFdel{result}\DIFdelend \DIFaddbegin \DIFadd{results}\DIFaddend .
With solo orderer service, throughput and latency reach a better trade-off around 300 tps and 1 second, respectively.
The workload necessary to get such results \DIFdelbegin \DIFdel{is around }\DIFdelend \DIFaddbegin \DIFadd{corresponds to }\DIFaddend 600 \DIFdelbegin \DIFdel{requested tsp}\DIFdelend \DIFaddbegin \DIFadd{simultaneous clients}\DIFaddend .
With a higher workload, performance degrades substantially\DIFdelbegin \DIFdel{, reaching }\DIFdelend \DIFaddbegin \DIFadd{.
We reach }\DIFaddend a throughput of around 260 tps and a high latency of 12 seconds \DIFdelbegin \DIFdel{with a workload of }\DIFdelend \DIFaddbegin \DIFadd{when testing a workload from }\DIFaddend 1,200 \DIFdelbegin \DIFdel{requested transactions .
}\DIFdelend \DIFaddbegin \DIFadd{clients.
...
\DIFaddend In turn, the BFT orderer service with four replicas performs better.
It reaches the best trade-off with throughput around 380 tps and latency in 2 seconds with the same workload of 600 \DIFdelbegin \DIFdel{requested}\DIFdelend \DIFaddbegin \DIFadd{simultaneous clients}\DIFaddend .
The BFT orderer also keeps throughput in a steady state about 360 tps even with a workload of 1,200 \DIFdelbegin \DIFdel{tsp}\DIFdelend \DIFaddbegin \DIFadd{clients}\DIFaddend , presenting only a slight increase in latency of 1 second.
\DIFdelbegin \DIFdel{Also, }\DIFdelend \DIFaddbegin \DIFadd{BFT orderer optimizes latency by discarding the exceeding of transactions after reaching the max throughput.
...
\DIFaddend BFT orderer service \DIFaddbegin \DIFadd{also includes an important aspect.
It }\DIFaddend enables a truly distributed consensus service, once each replica belongs to a different organization.
\DIFaddbegin \DIFadd{Although the number of replicas impacts performance, it aggregates security by preventing collusion attacks.
}\DIFaddend 
...
\bibliography{IEEEabrv,referencias062018}
%
