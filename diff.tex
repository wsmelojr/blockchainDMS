...
%DIF ADD tim.tex         Thu Jul 19 14:33:29 2018
%DIF < %% bare_conf.tex
%DIF -------
%% bare_jrnl.tex %DIF > 
%DIF -------
...
\title{\DIFdelbegin \DIFdel{How }\DIFdelend \DIFaddbegin \DIFadd{Using }\DIFaddend Blockchains \DIFdelbegin \DIFdel{can improve }\DIFdelend \DIFaddbegin \DIFadd{for implementing Distributed }\DIFaddend Measuring \DIFdelbegin \DIFdel{Instruments Regulation and Control}\DIFdelend \DIFaddbegin \DIFadd{Systems}\DIFaddend }
...
\thanks{A. Bessani and N. Neves are with LaSIGE, Faculdade de Ciências, Universidade de Lisboa, Portugal.}%DIF >  <-this % stops a space
...
We work with the hypothesis that such difficulties should be overcome with alternative approaches that simplify MI design while employing strategies for decentralizing metrological supervision. 
...
\DIFaddbegin \DIFadd{More recently, a few works have proposed blockchain applications in legal metrology which include decentralized audit, mechanisms for software loading, PKI for MI manufacturers and Distributed Measuring Systems (DMS) \mbox{%DIFAUXCMD
\cite{Peters2018,MeloJr.2017}}%DIFAUXCMD
.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{The paper extends the ideas presented in our previous work in \mbox{%DIFAUXCMD
\cite{MeloJr.2018a}}%DIFAUXCMD
. 
}\DIFaddend We start from preliminary concepts already consolidated in MI regulation and control. 
...
\DIFaddbegin \item \DIFadd{We develop a practical case study using HyperLedger Fabric framework. We create a blockchain network which implements legally relevant software using smart contracts for measuring vehicle speed. We also present consistent results that demonstrate our idea feasibility.
...
Initially associated to crypto-currency markets due to Bitcoin popularity \cite{Zheng2017}, blockchain-based architectures have been proposed for a wide set of application areas including sensors networks, internet of things, smart cities, among others \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Consequently, block $n$ can not be changed without also modifying all subsequent blocks $n + i, ..., n + k$ \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
In turn, integrity and availability are ensured by consensus among the peers, preventing the whole chain from being modified and requiring an agreement about any block to be appended to the ledger \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
is probably the most well-known blockchain implementation supporting that. }\DIFdelend %DIF > Ethereum \cite{Christidis2016} is probably the most well-known blockchain implementation supporting that. 
...
In this section, a security analysis is done by comparing the measuring models discussed in the previous section, considering the attacks and the metrological assurance framework BMAF previously described. We demonstrate how BMAF requirements and activities are impacted when applied on traditional MI and DMS. 
...
%DIFDELCMD < %%%
\DIFdel{Although blockchains-based DMS is a promising approach }\DIFdelend \DIFaddbegin \DIFadd{Practical Experiment}}

\subsection{\DIFadd{Implementing vehicle speed meters as a distributed measuring system}}
...
\DIFadd{We develop a practical experiment to demonstrate our proposal feasibility.
It consists of a vehicle speed DMS using blockchains (Figure \ref{f:speedmeter}).
Vehicle speed meters are efficient solutions for estimating the use of public roads, generating traffic statistics and enforcing speed limits for drivers \mbox{%DIFAUXCMD
...
\cite{EuropeanCooperationinLegalMetrologyWELMEC2015} }%DIFAUXCMD
defines this classification as instruments which run their software in universal computer hardware.
...
We collect the speed meter LR features condensed in a simple tamper proofing hardware that uses two inductive sensors for capture the vehicle's magnetic profile \mbox{%DIFAUXCMD
...
\subsection{\DIFadd{Architecture model using Hyperledger Fabric}}
\DIFadd{Our experiment uses Hyperledger Fabric \mbox{%DIFAUXCMD
\cite{Androulaki2018} }%DIFAUXCMD
for developing a permissioned blockchain where the peers cooperate to store measurements and execute LR software.
Fabric is an open source blockchain platform that includes two concepts which are very helpful for implementing our idea: }\emph{\DIFadd{endorsers}} \DIFadd{and }\emph{\DIFadd{security policies}}\DIFadd{.
}

\DIFadd{Endorsers are peers which effectively execute smart contracts, which are called chaincodes in Fabric.
...
\DIFadd{In complement, security policies can work as a protection mechanism against collusion attacks and also implement metrological surveillance.
...
\DIFadd{Once we propose a permissioned blockchain, the consensus plays an important role in our experiment.
Fabric referee consensus as }\emph{\DIFadd{orderer service}}\DIFadd{.
We perform our experiment with two different types of orderer services: the }\emph{\DIFadd{solo orderer}} \DIFadd{and the }\emph{\DIFadd{BFT orderer}}\DIFadd{.
The solo orderer service is native from Fabric distribution and very practical to implement tests and concept proofs.
...
In turn, the BFT orderer is developed by Sousa et al. \mbox{%DIFAUXCMD
\cite{Sousa2018} }%DIFAUXCMD
for tolerating Byzantine failures.
Thus one can configure BFT orderer to work with more than one replica, with different organizations controlling each one of them.
Such an approach results in a decentralized consensus while provides security against collusion attacks.
}
...
\DIFadd{In Fabric, a chaincode instantiation includes the notification of endorsers that they may execute that chaincode.
...
\DIFadd{After Inmetro instantiates the LR chaincode, any MI owner can contract computing services from LaSIGE organization for executing the respective LR software.
...
\subsection{\DIFadd{Testing some attacks}}
...
\DIFadd{Since the blockchain ledger already contains the chaincode fingerprint of every instantiated chaincode, a legitimate endorser automatically checks the LR chaincode integrity on deploy.
...
\DIFadd{One should remember that, in the proposed DMS, organizations compute measurements as an independent service (i.e., they do not have any advantage or rewarded regarding the measurement result).
...
We try to estimate the blockchain peers best achievement without to consider network communication issues.
...
, there was in 2016 something around one thousand speed meters spread along the city.
...
\cite{Androulaki2018} }%DIFAUXCMD
benchmarks Fabric performance in something around 2,000 and 3,000 tsp.
...
Fabric standard distribution runs over }\emph{\DIFadd{docker containers}} \DIFadd{so that each physical node can host several peers.
...
The first one is the native Fabric's solo orderer service, which is provided primarily for testing and does not have a practical use.
...
\DIFadd{The client transactions load is simulated using four nodes from a Dell PowerEdge R300, each one with an Intel Xeon Processor L5410 with 2.33 GHz and 8 GB of RAM.
...
Client containers are responsible for simulating transactions.
At the test execution, each client tries to produce a workload of $n$ concurrent transactions per second.
In fact}\DIFaddend , there are some \DIFdelbegin \DIFdel{challenges that }\DIFdelend \DIFaddbegin \DIFadd{difficulties in effectively generating this number of concurrent transactions without modifying Fabric implementation.
Since we use Fabric as an }\emph{\DIFadd{off-the-shelf solution}}\DIFadd{, we simulate each transaction as an independent process, which requires massive hardware resources.
...
\DIFadd{We also test scenarios with different orderer services.
The first scenario uses the solo orderer service provide by Fabric.
The second scenario works with the BFT orderer service with four replicas, each one of them under control of a different organization.
The comparing between these scenarios is important for evaluating performance aspects.
}
...
\DIFadd{Figures \ref{f:latth} and \ref{f:latthbft4} depict our tests result.
With solo orderer service, throughput and latency reach a better trade-off around 300 tps and 1 second, respectively.
The workload necessary to get such results is around 600 requested tsp.
With a higher workload, performance degrades substantially, reaching a throughput of around 260 tps and a high latency of 12 seconds with a workload of 1,200 requested transactions.
In turn, the BFT orderer service with four replicas performs better.
It reaches the best trade-off with throughput around 380 tps and latency in 2 seconds with the same workload of 600 requested.
The BFT orderer also keeps throughput in a steady state about 360 tps even with a workload of 1,200 tsp, presenting only a slight increase in latency of 1 second.
Also, BFT orderer service enables a truly distributed consensus service, once each replica belongs to a different organization.
}

\DIFadd{One can observe that the results point out a difficulty in dealing with the estimated peak demand of 800 tps.
However, we understand that the blockchain can absorb such demand along the day since the number of transactions goes down after the peak hours.
...
We highlight the following main issues that shall be discussed by researches in future works}\DIFaddend :
...
\DIFdelbegin \DIFdel{Benchmark tests in Sousa }\DIFdelend \DIFaddbegin \DIFadd{In our tests with Hyperledger Fabric, for instance, we reach a max throughput of 380 tps, although Androulaki }\DIFaddend et al. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sousa2017} }%DIFAUXCMD
indicate that the best available blockchain platforms are able to reach a peak around of 2000 transactions/second. 
Such performance is certainly }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Androulaki2018} }%DIFAUXCMD
points out a performance around of 2,000 tps in their benchmark. 
However, even such performance is indeed }\DIFaddend not enough to meet the demand for measurements on a smart grid, for instance.
...
\section{Conclusion}
In this paper\DIFaddbegin \DIFadd{, }\DIFaddend we discussed how blockchains can be used to support DMS. 
...
\DIFdelbegin \DIFdel{Despite }\DIFdelend \DIFaddbegin \DIFadd{We demonstrated that properties by implementing vehicle speed meter DMS using Hyperledger Fabric.
...
Future works shall \DIFdelbegin \DIFdel{bring experimental results and }\DIFdelend \DIFaddbegin \DIFadd{continue developing }\DIFaddend technical strategies for addressing and providing solutions for such difficulties.
...
\DIFaddend % \subsection{The measuring Big Data}
