...
%DIF DEL bare_conf.tex   Mon Jul 16 17:39:55 2018
%DIF ADD tim.tex         Tue Jul 24 15:00:42 2018
%DIF < %% bare_conf.tex
%DIF -------
%% bare_jrnl.tex %DIF > 
%DIF -------
...
%DIF 5-6c5
%DIF < %% See:
%DIF < %% http://www.michaelshell.org/
%DIF -------
%% see http://www.michaelshell.org/ %DIF > 
%DIF -------
...
%DIF -------
 %DIF > 
 %DIF > 
\documentclass[journal]{IEEEtran} %DIF > 
%DIF -------
...
%DIF > \title{How Blockchains can improve Measuring Instruments Regulation and Control}
\title{\DIFdelbegin \DIFdel{How }\DIFdelend \DIFaddbegin \DIFadd{Using }\DIFaddend Blockchains \DIFdelbegin \DIFdel{can improve }\DIFdelend \DIFaddbegin \DIFadd{to Implement Distributed }\DIFaddend Measuring \DIFdelbegin \DIFdel{Instruments Regulation and Control}\DIFdelend \DIFaddbegin \DIFadd{Systems}\DIFaddend }
...
\DIFaddbegin \author{\DIFadd{Wilson~S.~Melo~Jr, %DIF > ~\IEEEmembership{Member,~IEEE,}
        Alysson~Bessani, %DIF > ~\IEEEmembership{Fellow,~OSA,}
        Nuno~Neves, %DIF > ~\IEEEmembership{Life~Fellow,~IEEE}% <-this % stops a space
        Altair~Santin
        and Luiz~F.R.C.~Carmo
}\thanks{W. Melo Jr. and L.F.R.C. Carmo are with the National Institute of Metrology, Quality and Technology, Duque de Caxias, RJ, Brazil; and with the Federal University of Rio de Janeiro, Brazil.}%DIF >  <-this % stops a space
\thanks{A. Bessani and N. Neves are with LaSIGE, Faculdade de Ci\^encias, Universidade de Lisboa, Portugal.}%DIF >  <-this % stops a space
\thanks{A. Santin is with Pontifical Catholic University of Parana, Curitiba, PR, Brazil.}%DIF >  <-this % stops a space
%DIF > \thanks{Manuscript received July 28, 2018; revised Xxxx NN, NNNN.}
}

\DIFaddend % conference papers do not typically use \thanks and this command
...
%DIF >  The paper headers
%DIF > \markboth{IEEE Transactions on Instrumentation and Measurements, Special Issue I2MTC, July~2018}
\DIFaddbegin \markboth{How Blockchains can improve Measuring Instruments Regulation and Control (extended version)}{}
%DIF > {Melo Jr. \MakeLowercase{\textit{et al.}}: How Blockchains can improve Measuring Instruments Regulation and Control (extended version)}
%DIF >  The only time the second header will appear is for the odd numbered pages
%DIF >  after the title page when using the twoside option.
%DIF >  
%DIF >  *** Note that you probably will NOT want to include the author's ***
%DIF >  *** name in the headers of peer review papers.                   ***
%DIF >  You can use \ifCLASSOPTIONpeerreview for conditional compilation here if
%DIF >  you desire.


\DIFaddend % make the title area
\maketitle

% As a general rule, do not put math, special symbols or citations
% in the abstract
\begin{abstract}
In the last years, measuring instruments have become quite complex due to the integration of embedded \DIFdelbegin \DIFdel{hardware }\DIFdelend \DIFaddbegin \DIFadd{systems }\DIFaddend and software components and the increasing aggregation of new features. 
...
In this work, we evaluate \DIFdelbegin \DIFdel{how blockchains can help }\DIFdelend \DIFaddbegin \DIFadd{the use of blockchains as a resource }\DIFaddend to overcome such challenges.
\DIFdelbegin \DIFdel{We propose }\DIFdelend %DIF > how blockchains can help to overcome such challenges. 
\DIFaddbegin \DIFadd{We start with }\DIFaddend a conceptual model for implementing measuring instruments in a distributed blockchain-based architecture, and compare it with traditional measuring instruments and distributed measuring models discussed in previous works. 
We also \DIFdelbegin \DIFdel{develop }\DIFdelend \DIFaddbegin \DIFadd{made }\DIFaddend a security analysis, demonstrating that blockchains-based measuring systems can impact \DIFdelbegin \DIFdel{how }\DIFdelend \DIFaddbegin \DIFadd{the way }\DIFaddend measuring instruments are used in consumer relations \DIFdelbegin \DIFdel{, at the same time that improve security and simplify }\DIFdelend \DIFaddbegin \DIFadd{while improving security and simplifying }\DIFaddend metrological regulation and control.
\DIFdelbegin \DIFdel{At the end}\DIFdelend \DIFaddbegin \DIFadd{In the sequence, we implement a vehicle speed measuring system using the Hyperledger Fabric blockchain platform.
...
Finally}\DIFaddend , we point out the main challenges \DIFaddbegin \DIFadd{related to such an approach}\DIFaddend , suggesting alternatives and potential \DIFdelbegin \DIFdel{research lines }\DIFdelend \DIFaddbegin \DIFadd{issues }\DIFaddend for future works.
\end{abstract}
...
Measurement instruments (MI) are used in \DIFdelbegin \DIFdel{a diversity of applications }\DIFdelend \DIFaddbegin \DIFadd{many application domains }\DIFaddend including industry, commerce, energy, transportation, \DIFdelbegin \DIFdel{medical }\DIFdelend \DIFaddbegin \DIFadd{health }\DIFaddend care and environment protection \cite{RodriguesFilho2015}. 
Only in Europe, MI are responsible for an annual turnover of more than 500 billion Euros \cite{Esche2015}. 
...
MI also can be seen as elementary building blocks for new technologies such as internet of things and cyber physical systems (e.g., smart grids) \cite{RodriguesFilho2015,Esche2015,Camara2012,Boccardo2014,Peters2015,Oppermann2016}. 

MI are nowadays quite complex, since they are strongly based on embedded \DIFdelbegin \DIFdel{electronic and software }\DIFdelend \DIFaddbegin \DIFadd{systems }\DIFaddend and are often connected and accessible by the Internet \cite{Esche2015,Camara2012}. 
\DIFdelbegin \DIFdel{That creates }\DIFdelend %DIF > That creates
\DIFaddbegin \DIFadd{This kind of scenario might expose MI to }\DIFaddend security gaps which can be explored with malicious intent \cite{Boccardo2014,Peters2015}.
...
In turn, metrological supervision becomes difficult due to the \DIFdelbegin \DIFdel{high number of MI different modelsand versions}\DIFdelend \DIFaddbegin \DIFadd{diversity of MI models}\DIFaddend , the capillarity of their deployment and the limited resources owned by regulation agencies.

We work with the hypothesis that \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{the aforementioned }\DIFaddend difficulties should be overcome with alternative approaches that simplify MI design while employing strategies \DIFdelbegin \DIFdel{for decentralizing }\DIFdelend \DIFaddbegin \DIFadd{to decentralize }\DIFaddend metrological supervision. 
Such idea finds many aspects in common with a new trendy technology: \emph{blockchains} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Zheng2017}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Nakamoto2008}}%DIFAUXCMD
}\DIFaddend . 
A blockchain can be described as a distributed data structure which assures information integrity and authenticity while providing a platform for executing self-enforce software procedures, called \emph{smart contracts} \cite{Christidis2016}. Blockchain solutions have been very successful in financial applications (e.g., Bitcoin and Ethereum), which motivates innovative ideas using blockchains in different applications and knowledge areas \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Zheng2017,Christidis2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Nakamoto2008,Christidis2016}}%DIFAUXCMD
.
More recently, a few works have proposed blockchain applications in legal metrology, which include decentralized audit, mechanisms for software loading, PKI for MI manufacturers and Distributed Measuring Systems (DMS) \mbox{%DIFAUXCMD
\cite{Peters2018,MeloJr.2018a}}%DIFAUXCMD
}\DIFaddend .
...
\DIFaddbegin \DIFadd{This paper extends the ideas presented in our previous work \mbox{%DIFAUXCMD
\cite{MeloJr.2018a}}%DIFAUXCMD
.
%DIF > under the title ``How Blockchains can improve Measuring Instruments Regulation and Control''
}\DIFaddend We start from preliminary concepts already consolidated in \DIFaddbegin \DIFadd{Legal Metrology about }\DIFaddend MI regulation and control. 
...
\DIFaddend %Furthermore, this model impacts significantly attacks against MI since it restricts attackers capabilities.

Our main contributions can be summarized as follows:
 \begin{itemize} 
 \item We introduce the idea of DM using blockchains and describe its advantages when compared to traditional MI and other DM models. To the best of our knowledge, \DIFdelbegin \DIFdel{we are the first }\DIFdelend \DIFaddbegin \DIFadd{this is the first paper }\DIFaddend to describe a blockchains-based DMS.
 \item We propose an architectural model for implementing our idea, showing that MI and blockchains enable a new business model where the measuring process is \DIFdelbegin \DIFdel{seen as }\DIFdelend an independent service, which reduces conflict of interests.
 \item We \DIFdelbegin \DIFdel{develop }\DIFdelend \DIFaddbegin \DIFadd{present }\DIFaddend a security analysis, showing that our model improves MI security since it constrains the attacker capabilities, thus simplifying MI regulation and control.
 %DIF < \item We implemented a practical case study based on HyperLedger Fabric framework, which presents initial results and demonstrates our idea feasibility.
 %DIF < \item We point out challenges that shall be addressed in future works using blockchains in measuring applications.
\DIFaddbegin \item \DIFadd{We develop a practical case study using the Hyperledger Fabric \mbox{%DIFAUXCMD
\cite{Androulaki2018} }%DIFAUXCMD
platform. We create a blockchain network that implements legally relevant software using smart contracts for measuring vehicle speed. We also present results that demonstrate the feasibility of our idea.
 }\item \DIFadd{We point out challenges that shall be addressed in future works using blockchains in measuring applications.
...
Usually, legal metrology regulations are defined by government agencies or international committees. Once a regulation directive is approved, it traditionally establishes a set of requirements and activities for providing MI metrological assurance \cite{RodriguesFilho2015}. Activities are classified in two levels. The first \DIFdelbegin \DIFdel{level }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend is related to legal control and includes MI type approval, validation and verification. The second level activities covers metrological supervision, including quality, \DIFdelbegin \DIFdel{marketing }\DIFdelend \DIFaddbegin \DIFadd{market }\DIFaddend and field surveillance. Activities can be executed by different parties according to the directives adopted in regulation. Usually, notified bodies\footnote{Notified bodies are public or private parties organized for verifying MI.} are designated to assert MI conformity in both activity levels \cite{Esche2015,Oppermann2016}. 
...
When one talks about electronic and software controlled MI, legal metrology activities can demand more complex procedures and specialized knowledge. Usually, regulation adopts security requirements and good practices from \DIFdelbegin \DIFdel{well consolidated }\DIFdelend \DIFaddbegin \DIFadd{well-known }\DIFaddend technical standards \cite{Esche2015,Peters2015,Luchsinger2008}. The \emph{OIML\footnote{OIML is the International Organization of Legal Metrology.} D 31(E)} document \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{InternationalOrganizationofLegalMetrologyOIML2008} }%DIFAUXCMD
}\DIFaddend and \emph{WELMEC\footnote{WELMEC is the European committee to promote cooperation in the field of legal metrology.} Software Guide 7.2} \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{EuropeanCooperationinLegalMetrologyWELMEC2015} }%DIFAUXCMD
}\DIFaddend are probably the more widespread standards for software controlled MI design, deployment and inspection. Both documents are taken as reference by metrological agencies, notified bodies and manufacturers in different countries \cite{Camara2012,Peters2015}.
...
Distributed measuring (DM) is a concept supported by previous works \DIFaddbegin \DIFadd{and correlated ideas}\DIFaddend . 
...
Initially associated to crypto-currency markets due to Bitcoin popularity \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Zheng2017}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Nakamoto2008}}%DIFAUXCMD
}\DIFaddend , blockchain-based architectures have been proposed for a wide set of application areas including \DIFdelbegin \DIFdel{sensors }\DIFdelend \DIFaddbegin \DIFadd{sensor }\DIFaddend networks, internet of things, smart cities, among others \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Zheng2017,Christidis2016,Vukolic2017a}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Christidis2016}}%DIFAUXCMD
}\DIFaddend . 
...
Consequently, block $n$ \DIFdelbegin \DIFdel{can not }\DIFdelend \DIFaddbegin \DIFadd{cannot }\DIFaddend be changed without also modifying all subsequent blocks $n + i, ..., n + k$ \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sousa2017}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Sousa2018}}%DIFAUXCMD
}\DIFaddend . 
Being a decentralized model, blockchains availability does not depend on third parties, which can greatly save costs\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Zheng2017}}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > \cite{Zheng2017}.
}\DIFaddend In turn, integrity and availability are ensured by consensus among the peers, preventing the whole chain from being modified and requiring an agreement about any block to be appended to the ledger \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sousa2017,Vukolic2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Sousa2018,Vukolic2016}}%DIFAUXCMD
}\DIFaddend .
Blockchain platforms can be classified as \emph{permissionless}, when anybody can join to the network and participate in the network consensus, or \emph{permissioned}, when consensus is achieved by a set of known and identifiable peers \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Vukolic2016,Vukolic2017a}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Vukolic2016}}%DIFAUXCMD
}\DIFaddend . Usually, permissioned blockchains consensus protocols expend less computational resources and can reach better transaction latency and throughput\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sousa2017}}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF > \cite{Sousa2018}.
}\DIFaddend %REMOVIDO: One can say that blockchain enables trustless networks while using public key cryptographic for providing confidentiality and authenticity. 

A blockchain can store virtually any digital asset, from data to self-executing scripts, usually defined as \emph{smart contracts}. \DIFdelbegin \DIFdel{Ethereum \mbox{%DIFAUXCMD
\cite{Christidis2016} }%DIFAUXCMD
is probably the most well-known blockchain implementation supporting that. That }\DIFdelend %DIF > Ethereum \cite{Christidis2016} is probably the most well-known blockchain implementation supporting that. 
\DIFaddbegin \DIFadd{This }\DIFaddend makes blockchain not only a data storage \DIFdelbegin \DIFdel{architecture }\DIFdelend \DIFaddbegin \DIFadd{solution }\DIFaddend but also a complete distributed platform for proper and distributed automated workflow \cite{Christidis2016}.
...
However, an elementary set of requirements and activities are representative to a most of \DIFdelbegin \DIFdel{software controlled }\DIFdelend \DIFaddbegin \DIFadd{software-controlled }\DIFaddend MI in terms of security properties. 
\DIFdelbegin \DIFdel{Knowing that }\DIFdelend \DIFaddbegin \DIFadd{From that point of view}\DIFaddend , we define our MI security scope based on a generic attack model and its respective metrological assurance framework. Both are described \DIFdelbegin \DIFdel{as follows}\DIFdelend \DIFaddbegin \DIFadd{in this section}\DIFaddend . %Both consist of a minimal set of requirements and activities necessary for providing MI realiability.

\subsection{Attack model}
\DIFaddbegin \label{s:attackmodel}
\DIFaddend We consider a simple attack model that can be \DIFdelbegin \DIFdel{formulated }\DIFdelend \DIFaddbegin \DIFadd{built }\DIFaddend from MI use cases, accordingly to OIML D 31(E) and WELMEC 7.2 guides. MI are targeted by malicious entities trying to get undue economical advantages by tampering with measurements. 
Basically, the attacker capability consists in changing \DIFaddbegin \DIFadd{the }\DIFaddend MI expected behavior, tampering any LR component and compromising measurements reliability. 
...
\DIFdelbegin \DIFdel{That includes }\DIFdelend \DIFaddbegin \DIFadd{Attackers can be }\DIFaddend manufacturers, vendors, clients and other entities. 
...
Once a MI is deployed, vendors and clients can have access to its resources, exploring eventual failures and misbehaviors or changing sensitive parameters related to MI \DIFdelbegin \DIFdel{precision}\DIFdelend \DIFaddbegin \DIFadd{accuracy}\DIFaddend . 
Furthermore, modern MI usually provide interfaces for loading software \DIFdelbegin \DIFdel{upgrades and improvements. }\DIFdelend \DIFaddbegin \DIFadd{updates and upgrades. %DIF >  and improvements.
}\DIFaddend Such features can be explored by malicious vendors and clients for loading tampered LR software or for modifying critical \DIFaddbegin \DIFadd{MI }\DIFaddend parameters.
...
\DIFdelbegin \DIFdel{As a paradox}\DIFdelend \DIFaddbegin \DIFadd{However}\DIFaddend , despite their complexity, traditional MI software modules are usually monolithic systems \DIFdelbegin \DIFdel{. That }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Abreu2017}}%DIFAUXCMD
. 
This fact }\DIFaddend affects metrological assurance activities substantially, making MI regulation and control more expensive and complex, due to the follow aspects:
 \begin{itemize} 
\item Type approval can demand MI hardware and software evaluation and checking against a set of \DIFdelbegin \DIFdel{reliability requirements. Once }\DIFdelend \DIFaddbegin \DIFadd{integrity requirements. Since }\DIFaddend LR and NLR are usually tightly coupled, notified bodies leading type approval need to evaluate and attest \DIFdelbegin \DIFdel{th }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend compliance of all software modules. In some cases, LR software source code must be inspected for assuring their correctness.
...
\item Metrological supervision requires \DIFdelbegin \DIFdel{that notified bodies }\DIFdelend \DIFaddbegin \DIFadd{notified bodies to }\DIFaddend have sufficient staff to proceed with MI surveillance activities in both manufacturing and \DIFdelbegin \DIFdel{field. Albeit }\DIFdelend \DIFaddbegin \DIFadd{in the field. Although }\DIFaddend physical seals can be helpful to protect physical components, they are innocuous to protect software components.
...
\label{s:mi_cloud}
When LR and NLR components are properly separated in independent modules, one could run these modules in different \DIFdelbegin \DIFdel{hardware sets }\DIFdelend \DIFaddbegin \DIFadd{devices }\DIFaddend connected by well-defined interfaces. Such architecture leads to a DMS. For evaluating its properties, we take a cloud computing MI model inspired in \cite{Oppermann2016}. In this model, LR and NLR software are running as cloud services, outside of MI physical set (Figure~\ref{f:compare}-B). We assume MI communicates with the cloud services using a secure channel (e.g., TLS) and that side channel attacks are infeasible.

When traditional and cloud models are compared, one can observe that the distributed architecture simplifies MI devices. MI practically do not include software components anymore once both LR and NLR software modules are running in the cloud. In practice, MI is now set up as a blend of sensors \DIFdelbegin \DIFdel{, AD convertersand a communication interface that ables }\DIFdelend \DIFaddbegin \DIFadd{and AD converters.
A communication interface allows the }\DIFaddend MI to send sensing raw data to the cloud measuring system. 
...
In any case, a significantly amount of software is moved \DIFaddbegin \DIFadd{from the MI }\DIFaddend to the cloud and works as a service. 
...
This idea fits \DIFdelbegin \DIFdel{perfectly }\DIFdelend \DIFaddbegin \DIFadd{very well }\DIFaddend in the blockchain \DIFdelbegin \DIFdel{architecture}\DIFdelend \DIFaddbegin \DIFadd{model}\DIFaddend . 
Smart contracts can be used for computing measurements based on sensing information. 
However they are coded by different parties that do not have conflicts of interests related to the measured quantity. Whatever the measurement result, these parties shall be rewarded by a \DIFdelbegin \DIFdel{fixed value. 
That }\DIFdelend \DIFaddbegin \DIFadd{preset value. 
This }\DIFaddend motivates new players to \DIFdelbegin \DIFdel{compete by more efficient measuring algorithmsonce as faster they execute, more credits they earn.
}\DIFdelend \DIFaddbegin \DIFadd{provide better measuring algorithms.
%DIF > once as faster they execute, more credits they can earn.
}\DIFaddend Additionally, such strategy creates incentives for keeping the blockchain network since that becomes profitable. 
\DIFdelbegin \DIFdel{One could say that such concept is an innovative idea once it }\DIFdelend %DIF > One could argue that such concept is an innovative idea once it breaks with the manner MI are traditionally used in consumer relations.
\DIFaddbegin \DIFadd{This also }\DIFaddend breaks with the manner \DIFdelbegin \DIFdel{how }\DIFdelend MI are traditionally used in consumer relations.
...
%The security analysis is summarized at.
...
The BMAF protection requirements are still necessary, however \DIFaddbegin \DIFadd{requirements }\DIFaddend R2 and R3 \DIFdelbegin \DIFdel{requirements }\DIFdelend are applied on \DIFdelbegin \DIFdel{LR software cloud implementation}\DIFdelend \DIFaddbegin \DIFadd{the LR software implemented in the cloud}\DIFaddend . 
...
In turn, A3 also becomes less expensive because LR software identification and integrity check are executed against cloud servers\DIFaddbegin \DIFadd{, }\DIFaddend which are far fewer \DIFdelbegin \DIFdel{in number than }\DIFdelend \DIFaddbegin \DIFadd{than the }\DIFaddend deployed MI. 
...
Blockchain peers \DIFdelbegin \DIFdel{can not }\DIFdelend \DIFaddbegin \DIFadd{cannot }\DIFaddend execute a different smart contract code\DIFaddbegin \DIFadd{, }\DIFaddend otherwise blockchain security assumptions will be violated.
...
Once LR software is produced by players which are exempted from conflict of interests, many activities related to \DIFdelbegin \DIFdel{assure }\DIFdelend \DIFaddbegin \DIFadd{the assurance of }\DIFaddend software correctness and integrity are made simpler \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend even unnecessary. 
The blockchain security properties plays an important role in this context. 

\section{\DIFdelbegin \DIFdel{Challenges Ahead}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdel{Although blockchains-based DMS is a promising approach }\DIFdelend \DIFaddbegin \DIFadd{Case Study}}

\subsection{\DIFadd{Vehicle speed meters as a distributed measuring system}}

\DIFadd{We develop an experiment to demonstrate our proposal feasibility.
It consists of a vehicle speed DMS using blockchains (Figure \ref{f:speedmeter}).
These meters are efficient solutions for estimating vehicle speed on public roads, generating traffic statistics and enforcing speed limits for drivers \mbox{%DIFAUXCMD
\cite{Ki2006,S2011}}%DIFAUXCMD
.
...
\DIFadd{Vehicle speed meters are usually built as }\emph{\DIFadd{Type-U instruments}}\DIFadd{. 
WELMEC 7.2 \mbox{%DIFAUXCMD
\cite{EuropeanCooperationinLegalMetrologyWELMEC2015} }%DIFAUXCMD
defines this classification as instruments that run their software in universal computer hardware.
...
\DIFadd{We implement such solution creating a blockchain network for distributed measuring.
We condense the speed meter's LR features in a simple tamper proofing hardware that uses two inductive sensors to capture the vehicle's magnetic profile \mbox{%DIFAUXCMD
\cite{S2011}}%DIFAUXCMD
.
...
\begin{figure}[!t]
\centering
\includegraphics[width=.45\textwidth]{speedmeter} %DIF > [width=2.5in]
\caption{\DIFaddFL{The blockchain-based vehicle speed DMS}}
\label{f:speedmeter}
\end{figure}

\subsection{\DIFadd{Architecture using Hyperledger Fabric}}
\DIFadd{Our prototype uses Hyperledger Fabric \mbox{%DIFAUXCMD
\cite{Androulaki2018} }%DIFAUXCMD
as a permissioned blockchain, where the peers cooperate to store measurements and execute LR software.
Fabric is an open source blockchain platform that includes two concepts that are very helpful for implementing our idea: }\emph{\DIFadd{endorsers}} \DIFadd{and }\emph{\DIFadd{security policies}}\DIFadd{.
}

\DIFadd{Endorsers are peers that effectively execute smart contracts, which are called chaincodes in Fabric.
...
\DIFadd{In turn, security policies can set the way the network validates the measurement provided by endorsers.
...
\DIFadd{Since we resort to a permissioned blockchain, the consensus plays an important role in our experiment.
Fabric refers to consensus as an }\emph{\DIFadd{orderer service}}\DIFadd{.
We perform our experiment with two different types of orderer services: the }\emph{\DIFadd{solo orderer}} \DIFadd{and the }\emph{\DIFadd{Byzantine Fault-Tolerant (BFT) orderer}}\DIFadd{.
...
In turn, the BFT orderer \mbox{%DIFAUXCMD
\cite{Sousa2018} }%DIFAUXCMD
is fully replicated for tolerating Byzantine failures.
Thus, one can configure the BFT orderer with several replicas, with different organizations controlling each one of them.
Such approach employs a decentralized consensus provided by the BFT-SMaRt replication library \mbox{%DIFAUXCMD
\cite{Bessani2014}}%DIFAUXCMD
, thus providing security against collusion attacks.
}

\subsection{\DIFadd{Describing MI regulation and control activities}}
\DIFadd{We set up our vehicle speed DMS according to the scheme in Figure~\ref{f:scheme}.
...
\DIFadd{The speed meter manufacturer implements LR software as a chaincode. 
%DIF > Fabric supports chaincode written in the Go programming language.
%DIF > Figure X shows a simple LR algorithm for calculating speed example in Go.
...
\DIFadd{Our experiment assumes the implementation of legal metrology activities as follows.
...
\DIFadd{In Fabric, a chaincode instantiation includes the notification of endorsers that will execute that chaincode.
...
\DIFadd{After Inmetro instantiates the LR chaincode, any MI owner can contract computing services from the LaSIGE organization for executing the respective LR software.
...
\begin{figure}[!t]
\centering
\includegraphics[width=.45\textwidth]{hlfscheme} %DIF > [width=2.5in]
\caption{\DIFaddFL{Vehicle speed DMS solution scheme}}
\label{f:scheme}
\end{figure}

\subsection{\DIFadd{Security analysis}}
\DIFadd{We verify how our implementation offers countermeasures against common attacks associated with the capabilities described at the Section~\ref{s:attackmodel}.
...
\subsubsection{\DIFadd{An attacker tries to compromise the speed meter hardware integrity}}
\DIFadd{We conceive the speed meter hardware as a tamper-proof device.
...
\subsubsection{\DIFadd{An attacker tries to install a malicious chaincode in an endorser peer}}
\DIFadd{The blockchain ledger contains the fingerprint of every instantiated chaincode, and legitimate endorsers automatically check the LR chaincode integrity on deploy time.
...
\subsubsection{\DIFadd{An attacker can collude with an organization for loading malicious chaincode in endorser peers}}
\DIFadd{Security policies assure protection against collusion attacks.
...
\subsubsection{\DIFadd{An attacker has success in colluding with enough peers for injecting fraudulent measurements in the blockchains}}
\DIFadd{One should remember that, in the proposed DMS, organizations compute measurements as an independent service (i.e., they do not have any advantage or reward regarding the measurement result).
...
\section{\DIFadd{Performance issues}}
\DIFadd{An essential step in our experiment is the speed meter DMS performance evaluation.
...
\subsection{\DIFadd{Demand goals}}
\DIFadd{We estimate the experiment demand based on vehicles traffic real data.
...
(CET-SP) \mbox{%DIFAUXCMD
\cite{CompanhiadeEngenhariadeTrafego-CET2017}}%DIFAUXCMD
, there was in 2016 approximately one thousand speed meters spread along the city.
The same report analyses the vehicles flow on the main roads in the city and points out an average of 2}\DIFaddend ,\DIFdelbegin \DIFdel{there are some challenges that }\DIFdelend \DIFaddbegin \DIFadd{772 vehicles/hour (or 0.76 vehicles/sec) during rush hour.
...
Androulaki et al. \mbox{%DIFAUXCMD
\cite{Androulaki2018} }%DIFAUXCMD
benchmarks Fabric performance in something between 2,000 to 3,000 tsp.
...
%DIF > That is done on purpose
The objective is to evaluate the results that a speed meter solution owner can obtain by using Fabric.
}

\subsection{\DIFadd{Test environment setup}}
\DIFadd{Our blockchain network environment consists of 3 nodes from a Dell PowerEdge R410 cluster.
Each node has two CPUs Intel® Xeon® Processor E5520 with 2.27 GHz and 32 GB of RAM.
Fabric standard distribution version 1.1}\footnote{\url{http://hyperledger-fabric.readthedocs.io/en/release-1.1}} \DIFadd{runs over }\emph{\DIFadd{docker containers}}\DIFadd{, so that each physical node can host several peers.
...
\DIFadd{Regarding the orderer service, we test two different scenarios.
The first one is the native Fabric's solo orderer service, which is provided primarily for testing.
%DIF > and does not have a practical use.
The second scenario uses the orderer service developed by Sousa et al. \mbox{%DIFAUXCMD
\cite{Sousa2018}}%DIFAUXCMD
, which tolerates Byzantine failures.
%DIF > We configure this service to work with four replicas.
}

\DIFadd{The client transactions load is mimicked using four nodes from a Dell PowerEdge R300 cluster, each one with an Intel Xeon Processor L5410 with 2.33 GHz and 8 GB of RAM.
The transactions simulation uses real data from speed meters developed by the company Perkons SA}\footnote{\url{http://www.perkons.com}}\DIFadd{, which kindly granted a dataset for this experiment.
}

\subsection{\DIFadd{Test methodology}}
\DIFadd{The performance tests execute as follows.
...
We use Fabric as an }\emph{\DIFadd{off-the-shelf solution}}\DIFadd{, without any customization.
Each client instance corresponds to a container process that sends transactions to the blockchain.
%DIF > continually, which requires massive hardware resources.
We try to produce a maximum workload by increasing the number of clients.
%DIF > Reaching such conditions, we determine the blockchain's best throughput and latency.
}

%DIF > At the test execution, each client tries to produce a workload of $n$ concurrent transactions per second.
%DIF > In fact, there are some difficulties in effectively generating this number of concurrent transactions without modifying the Fabric implementation.
%DIF >  Even so, the measured throughput and latency are precise once they comprise the receiving of all simulated transactions by the orderer service.

\DIFadd{When a client process instance is created, it selects a vector of bytes containing the sensors raw data from the dataset mentioned above, for each transaction.
...
Such information gives the throughput and latency of the system. %DIF > estimates for the endorsement and orderer services.
%DIF > In turn, an independent peer in Inmetro organization provides the blockchain with throughput estimative. 
%DIF > We keep checking this peer once every 10 seconds, counting how many transactions were committed to the ledger.
}

%DIF >  We also test scenarios with different orderer services.
%DIF >  The first scenario uses the solo orderer service provide by Fabric.
%DIF >  The second scenario works with the BFT orderer service with four replicas.
%DIF >  We consider that each replica is under control of a different organization.
%DIF >  The comparison between these scenarios is important for evaluating performance aspects.

\begin{figure}[!t]
\centering
\includegraphics[width=.45\textwidth]{chartsolo} %DIF > [width=2.5in]
\caption{\DIFaddFL{Throughput and latency using native Fabric solo orderer}}
\label{f:latth}
\end{figure}

%DIF >  \begin{figure}[!t]
%DIF >  \centering
%DIF >  \includegraphics[width=.45\textwidth]{chartbft1r} %[width=2.5in]
%DIF >  \caption{Throughput and latency using BFT orderer with one replica}
%DIF >  \label{f:latthbft1}
%DIF >  \end{figure}

\subsection{\DIFadd{Performance test results}}
\DIFadd{Figures \ref{f:latth} and \ref{f:latthbft4} depict our tests results for the system using the solo orderer and the BFT orderer with 4 replicas, respectively.
...
%DIF > However, this method compromises performance, resulting in low throughput and misusing of the MI communication channel.
}

\DIFadd{In turn, the BFT orderer service with four replicas performs better.
...
\DIFadd{BFT orderer service also includes an important aspect.
It enables a truly distributed consensus service, once each replica belongs to a different organization.
Although the number of replicas impacts performance \mbox{%DIFAUXCMD
\cite{Bessani2014}}%DIFAUXCMD
, it aggregates security by preventing collusion attacks.
}

\DIFadd{One can observe that our results point out a difficulty in dealing with the estimated peak demand of 800 tps.
...
\begin{figure}[!t]
\centering
\includegraphics[width=.45\textwidth]{chartbft4r} %DIF > [width=2.5in]
\caption{\DIFaddFL{Throughput and latency using BFT orderer with four replicas}}
\label{f:latthbft4}
\end{figure}

\section{\DIFadd{Challenges Ahead}}
\DIFadd{Although blockchains-based DMS is a promising approach, several challenges }\DIFaddend need to be addressed for their use.
\DIFdelbegin \DIFdel{The main ones as discussed as follow}\DIFdelend \DIFaddbegin \DIFadd{Some of them become very clear after we proceed with our practical experiment.
We highlight the following main issues that should be addressed in future works}\DIFaddend :
...
\DIFdelbegin \DIFdel{Benchmark tests in Sousa }\DIFdelend \DIFaddbegin \DIFadd{In our tests with Fabric, for instance, we reach a max throughput of 380 tps, although Androulaki }\DIFaddend et al. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sousa2017} }%DIFAUXCMD
indicate that the best available blockchain platforms are able to reach a peak around of 2000 transactions/second. 
Such performance is certainly not }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Androulaki2018} }%DIFAUXCMD
points out a performance more than of 2,000 tps in their benchmark. 
However, even such performance may not be }\DIFaddend enough to meet the demand for measurements on a smart grid, for instance.
\DIFdelbegin \DIFdel{As a possible solution, aggregated measurements can be used }\DIFdelend \DIFaddbegin \DIFadd{In such scenarios, solutions can require different workarounds and creative alternatives.
We recall the use of endorsers, an idea that we explored with success in our experiment.
Besides, one can try to use aggregated measurements }\DIFaddend for reducing transactions in a blockchain implementation. 
\DIFdelbegin \DIFdel{Smarter MI can also be tried }\DIFdelend \DIFaddbegin \DIFadd{Also, one can try smarter MI }\DIFaddend for determining transactions on demand.
\DIFdelbegin \DIFdel{Another idea is to use }\emph{\DIFdel{transaction endorsers}}%DIFAUXCMD
\DIFdel{, a concept introduced by HyperLedger Fabric \mbox{%DIFAUXCMD
\cite{Vukolic2017a,Sousa2017}}%DIFAUXCMD
. Endorsers can execute complex measuring computing, leaving only validation tasks for regular peers.
}\DIFdelend \DIFaddbegin 

%DIF > Another idea is to use \emph{transaction endorsers}, a concept introduced by HyperLedger Fabric \cite{Sousa2018}. Endorsers can execute complex measuring computing, leaving only validation tasks for regular peers.
\DIFaddend 
...
In a blockchain with \DIFaddbegin \DIFadd{a }\DIFaddend public ledger, this problem becomes more serious. 
One needs to establish an acceptable trade-off between privacy and efficiency.
\DIFdelbegin \DIFdel{Depending }\DIFdelend \DIFaddbegin \DIFadd{In our experiment, we faced such a problem with the vehicle license plate image and solved it by storing such information outside of the blockchain.
However, depending }\DIFaddend on the application scenario, privacy can require more sophisticated mechanisms for protecting or obfuscating identities, such as pseudonyms or identity protection layers. 
Permissioned blockchains \DIFdelbegin \DIFdel{can also }\DIFdelend constitute a suitable alternative once they contemplate an access control layer built into blockchain nodes \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Vukolic2017a,Vukolic2016}}%DIFAUXCMD
. 
Access policies can be constrained }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Vukolic2016}}%DIFAUXCMD
. 
One can also constrain access policies }\DIFaddend in such a manner that they satisfy privacy rules and restrictions.
...
Furthermore, attacks aiming communication (e.g., DDoS) represent an additional risk, although \DIFdelbegin \DIFdel{fully distributed systems }\DIFdelend \DIFaddbegin \DIFadd{decentralized systems such }\DIFaddend as blockchains are more resilient to such attacks than conventional cloud architectures.
...
\DIFdelbegin \DIFdel{Despite the fact that }\DIFdelend \DIFaddbegin \DIFadd{Even though }\DIFaddend sensors are small components which can be protected using physical seals, sensors authentication can be necessary to assure measuring reliability.

\section{Conclusion}
In this paper\DIFdelbegin \DIFdel{we discussed how }\DIFdelend \DIFaddbegin \DIFadd{, we discussed a way }\DIFaddend blockchains can be used to support DMS.
...
\DIFdelbegin \DIFdel{Despite }\DIFdelend \DIFaddbegin \DIFadd{We demonstrated those properties by implementing a vehicle speed meter DMS using Hyperledger Fabric.
Our results were consistent, and they support our idea feasibility. 
However, despite }\DIFaddend its promising application, blockchains pose several challenges that need to be faced. 
The main \DIFaddbegin \DIFadd{ones }\DIFaddend are related to the amount of data, privacy\DIFaddbegin \DIFadd{, communication }\DIFaddend and oracles authentication. 
Future works shall \DIFdelbegin \DIFdel{bring experimental results and }\DIFdelend \DIFaddbegin \DIFadd{continue developing }\DIFaddend technical strategies for addressing \DIFdelbegin \DIFdel{and providing solutions for }\DIFdelend such difficulties.
...
\DIFaddbegin \DIFadd{The authors also thank Perkons SA for kindly providing the speed meters dataset used in the experiments of this work.
}

\DIFaddend This work was partially sponsored by the \DIFdelbegin \DIFdel{EU-BR SecureCloud project (MCTI/RNP 3rd Coordinated Call); and }\DIFdelend by the Coordination for the Improvement of Higher Education Personnel (CAPES), grant 99999.008512/2014-0\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; by the EU-BR SecureCloud project (MCTI/RNP 3rd Coordinated Call); and }\DIFaddend by FCT through \DIFdelbegin \DIFdel{project LaSIGE }\DIFdelend \DIFaddbegin \DIFadd{the LASIGE Research Unit }\DIFaddend (UID/CEC/00408/2013) \DIFaddbegin \DIFadd{and the IRCoC project (PTDC/EEI-SCR/6970/2014)}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \bibliography{IEEEabrv,mythesis}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{IEEEabrv,referencias062018}
\DIFaddend %
