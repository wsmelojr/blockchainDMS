...
\title{Using Blockchains for implementing Distributed Measuring Systems}
...
\thanks{A. Bessani and N. Neves are with LaSIGE, Faculdade de Ciências, Universidade de Lisboa, Portugal.}% <-this % stops a space
...
The paper extends the ideas presented in our previous work in \cite{MeloJr.2018a}. 
We start from preliminary concepts already consolidated in MI regulation and control. 
...
WELMEC 7.2 \cite{EuropeanCooperationinLegalMetrologyWELMEC2015} defines this classification as instruments which run their software in universal computer hardware.
...
Fabric is an open source blockchain platform that includes two concepts which are very helpful for implementing our idea: \emph{endorsers} and \emph{security policies}.
...
Once we propose a permissioned blockchain, the consensus plays an important role in our experiment.
Fabric referee consensus as \emph{orderer service}.
...
Androulaki et al. \cite{Androulaki2018} benchmarks Fabric performance in something around 2,000 and 3,000 tsp.
...
The client transactions load is simulated using four nodes from a Dell PowerEdge R300, each one with an Intel Xeon Processor L5410 with 2.33 GHz and 8 GB of RAM.
...
Since we use Fabric as an \emph{off-the-shelf solution}, we simulate each transaction as an independent process, which requires massive hardware resources.
...
Figures \ref{f:latth} and \ref{f:latthbft4} depict our tests result.
...
One can observe that the results point out a difficulty in dealing with the estimated peak demand of 800 tps.
...
We demonstrated that properties by implementing vehicle speed meter DMS using Hyperledger Fabric.
