...
\markboth{How Blockchains can improve Measuring Instruments Regulation and Control (extended version)}{}
%{Melo Jr. \MakeLowercase{\textit{et al.}}: How Blockchains can improve Measuring Instruments Regulation and Control (extended version)}
...
In the sequence, we implement a vehicle speed measuring system using the Hyperledger Fabric blockchain platform.
...
Finally, we point out the main challenges related to such an approach, suggesting alternatives and potential issues for future works.
\end{abstract}
...
MI also can be seen as elementary building blocks for new technologies such as internet of things and cyber physical systems (e.g., smart grids) \cite{RodriguesFilho2015,Esche2015,Camara2012,Boccardo2014,Peters2015,Oppermann2018}. 
...
In turn, metrological supervision becomes difficult due to the diversity of MI models, their geographically spread over deployment and the limited resources owned by regulatory agencies.
...
Legal metrology embraces MI regulation and control. It is crucial to assure the measurements correctness \cite{RodriguesFilho2015} and regulate consumer relations \cite{Oppermann2018}.
...
The main ones are related to legal control MI and type approval, which can include documentation and code inspection, validation and verification; and metrological supervision, including quality, market and field surveillance.
...
The majority of security issues with MI are associated to economic undue advantages.
...
Measurement frauds against MI (such as scales, energy meters and fuel pumps) are very common in developing countries \cite{Luchsinger2008,Camara2012}.
Attacks can also intend to steal sensitive information and intellectual property \cite{Camara2012,Oppermann2018}.
In some cases they even threat people physical integrity (e.g., tampering measurements related to medical procedures) \cite{Boccardo2014}.
...
Although it is not a DM case, such approach suggests that part of the measurement computing can be done externally to the sphygmomanometer hardware core due to the use of a digital signature to check sensing data integrity and authenticity.
In turn, Peters et al. \cite{Peters2015} describes a MI security framework using virtual machines to get separation among \emph{legally relevant} (LR) and \emph{non-legally relevant} (NLR) software.\footnote{OIML D 31(E) and WELMEC 7.2 use LR to designate any component which can affect measuring final results, while NLR cannot do that.} The authors propose different virtual machines to execute LR and NLR functions and define secure interfaces for communicating among them. This approach is presented as an alternative to improve security and reduce MI complexity. Additionally, it enables virtualization using different hardware cores and consequently allows the implementation of \emph{Distributed Measuring Systems} (DMS). Lastly, a DM architecture using cloud computing is discussed by Oppermann et al. \cite{Oppermann2018}. The authors present advantages related to IT infrastructure cost-savings and the possibility of MI manufacturers to offer modern interconnected devices and features.
They also present a comprehensive example of how integrates energy meters in a DMS. 
...
In this work, we want to evaluate the security level of different measuring systems models and so point out the advantages and drawbacks of each model. 
We are especially interested in MI reliability and the required efforts for providing MI metrological assurance.
Metrological requirements and activities are very particular for different MI classes. 
However, a simple set of requirements and activities are representative to a most of software-controlled MI concerning security properties. 
...
The claimed reasons are costs, computational resources restrictions or the existence of a software legacy. 
...
\item Metrological supervision requires notified bodies to have sufficient staff to proceed with MI surveillance activities in both manufacturing and the field. Although physical seals can be helpful to protect physical components, they are innocuous to protect software components.
...
In any case, a significant amount of software is moved from the MI to the cloud, and it works as a service. 
Consequently, LR and NLR software can be scaled accordingly to the demand.

A DMS also enables easily software separation.
This happens because LR and NLR software do not run over a monolithic platform anymore.
That encourages manufacturers to eliminate coupling between LR and NLR software, which consequently makes LR software less complex.
...
Also, the blockchain also would support the execution of LR (and even NLR) software using smart contracts, which process information from sensors and generate a consolidated measurement value. 
The integrity of measurements and LR software (as smart contracts) is preserved by blockchain inherent properties \cite{Zheng2017}.
...
Thus, one can expect that a blockchain model will require the contribution of different parties interested in the measurement activities, and consequently it need to be idealized following a different philosophy.
...
This concept also breaks with the manner MI are traditionally used in consumer relations.
...
However, the more organizations take part in the consensus, more expensive and unfeasible these attacks become.
...
Documentation analysis, code inspection, and software validation and verification need to comprise all components and software modules.
...
In A1, the document analysis and the code inspection of LR software running in the cloud are expected to require fewer efforts than embedded software evaluation. 
...
Regarding the activities, although A1 and A2 are still necessary, they should become much simpler when compared to the required effort in the other models.
...
Once LR software is produced by players who are exempted from conflict of interests, many activities related to the assurance of software correctness and integrity are made simpler or even unnecessary. 
...
In this section, we develop an experiment to demonstrate our proposal feasibility.
...
This experience provides valuable information that supports our analysis in this section.  

Developing countries are essentially guided for legal metrology policies related to fraud detection and avoidance \cite{Luchsinger2008,Camara2012,RodriguesFilho2015}.
...
In this aspect, the blockchain-based DMS can introduce promising advantages.
It simplifies the metrological assurance framework activities, at the same time that keeps many of the advantages from a DMS concerning performance and costs saving.
...
That is the case of the speed meters evaluated for us in Inmetro.
So they are representative cases of the traditional MI model described in this work.

Another aspect is that speed meters owners usually deploy their equipment at far places along roads spread over large geographic areas.
...
Thus, all those aspects make speed meters strong candidates for solutions that help to separate LR and NLR software, while offering easiness for legal metrology activities such as documentation analysis, code inspection, V\&V, and field surveillance.
...
This software size statistically suggests the remaining of about 18 software defects and, consequently, a high probability of the existence of potential vulnerability.
Such LR software size is a consequence of the strong coupling which imposes dependences among LR and NLR modules.
...
Its LR software consists of a Go language smart contract that requires no more than 200 LOCs.
Such size points out an estimative of no more than two remaining software defects, which also reduces the chances of potential vulnerabilities.
...
However, we understand that the NLR software also can be provided by independent remote services.
...
Concerning the demand by connectivity, we can stat that both solutions are similar.
Although the traditional MI can send information in batch mode, the expected information amount is practically the same than in the blockchain-based DMS.
In turn, despite the extensive use of cryptography in a blockchain application, that does not represent a significative overhead regarding the amount of propagated information.
Roughly speaking, the use of connectivity resources depends on the number of detected vehicles and not from the adopted solution model.
...
So we can stat that the $MTBF_h$ of the simple MI hardware used int the blockchain-based DMS is expected to be higher than the $MTBF_H$ of traditional MI.
...
However, we can stat two crucial aspects:
\begin{itemize}
 ...
 \item The blockchain-based DMS is expected to present a better availability ratio due to its replicas that provide fault tolerance to the system.
\end{itemize}
...
Future works shall include a complete risk analysis of our blockchain-based model, as so as new strategies for addressing the challenges discussed at the end.
